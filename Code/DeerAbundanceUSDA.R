...
abun.dt<-CleanData(usda.list$Abun)$cleandat
climindex.dt<-lapply(climindex.usda[c(1:3,5:7)],function(x){x<-CleanData(x)$cleandat;x})
...
winter.clim.usda.dt<-lapply(winter.clim.usda,function(x){x<-CleanData(x,normalize=T)$cleandat;x})
...
win.mei.dt<-CleanData(climindex.usda$WinterMEI,normalize=T)$cleandat
abun.dt<-CleanData(usda.list$Abun,normalize=T)$cleandat
...
snowd.dt<-CleanData(winter.clim.usda$Snwd,normalize=T)$cleandat
abun.dt<-CleanData(usda.list$Abun,normalize=T)$cleandat #truncate abundance by snow
...
sum.mei.dt<-CleanData(climindex.usda$SummerMEI,normalize=T)$cleandat
abun.dt<-CleanData(usda.list$Abun,normalize=T)$cleandat
...
win.pdo.dt<-CleanData(climindex.usda$WinterPDO,normalize=T)$cleandat
abun.dt<-CleanData(usda.list$Abun,normalize=T)$cleandat
...
abun.dt<-CleanData(abun,normalize=T)$cleandat
hunters<-usda.list$Hunters[(!row.names(usda.list$Hunters)%in%cwd.usda),12:36]
hunters.dt<-CleanData(hunters,normalize=T)$cleandat
...
usda.list.dt<-lapply(usda.list[c(7,9)],function(x){x<-CleanData(x[,!is.na(colSums(usda.list$Crashes))])$cleandat;x})
...
usda.list.dt<-lapply(usda.list[c(7,11)],function(x){x<-CleanData(x[,!is.na(colSums(usda.list$AdjDVC))])$cleandat;x})
