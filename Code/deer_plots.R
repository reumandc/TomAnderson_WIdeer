...
par(mfrow=c(3,1),mgp=c(1.5,0.5,0),mai=c(0.25,0.75,0.1,0))
...
contour(x=dvc.wpmf$times,y=log2(dvc.wpmf$timescales),z=Mod(dvc.wpmf$values),levels=q,drawlabels=F,lwd=2,
        xaxs="i",xaxt="n",yaxt="n",xaxp=c(0,1,5),las = 1,frame=F)
syncexpplot(resp.wmf=wmfwt(wlm_dvc$wts[[1]]),exp.sync = predsync(wlm_dvc)[[3]],1987:2016,
...
wpdo.wmf<-wsyn::wt(cleandat(climindex$WinterPDO[1,],clev=5,times=minyear:maxyear)$cdat,times=minyear:maxyear)
deer_wmfplot(wpdo.wmf,colorbar=T,xlab="",ylab="",power=T)
mtext("A)",adj=0.05,line=-1.2,side=3,font=2)
spdo.wmf<-wsyn::wt(cleandat(climindex$SummerPDO[1,],clev=5,times=minyear:maxyear)$cdat,times=minyear:maxyear)
...
wmei.wmf<-wsyn::wt(cleandat(climindex$WinterMEI[1,],clev=5,times=minyear:maxyear)$cdat,times=minyear:maxyear)
deer_wmfplot(wmei.wmf,colorbar=T,xlab="Year",ylab="",power=T)
mtext("E)",adj=0.05,line=-1.2,side=3,font=2)
smei.wmf<-wsyn::wt(cleandat(climindex$SummerMEI[1,],clev=5,times=minyear:maxyear)$cdat,times=minyear:maxyear)
deer_wmfplot(smei.wmf,colorbar=T,xlab="Year",ylab="",power=T)
