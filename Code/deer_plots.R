...
abun.dt<-CleanData(cty.list$Abun)$cleandat
dvc.dt<-CleanData(cty.list$Crashes[,!is.na(colSums(cty.list$Crashes))])$cleandat
...
syncexpplot(resp.wmf=wmfwt(abun.wt$wave.array),exp.sync = model.es$exp.sync,times=1981:2016,abun.wt$timescales,smallplot=c(0.95,0.99,0.05,0.95),lwd=2)
...
mtext(text = "Timescales",line = 1.3,side = 2,cex=1.25)
par(fig=c(ywd/tot.wd,(ywd+pan.wd)/tot.wd,xht/tot.ht,(xht+pan.ht)/tot.ht),mai=c(0,0,0,0),new=T)
syncexpplot(resp.wmf=wmfwt(dvc.wt$wave.array),exp.sync = dvcabun.es$pred.wmf,1987:2016,dvcabun.es$timescales,ylab = "",xlab="Year",smallplot=c(0.95,0.99,0.05,0.95),lwd=2)
...
phaseplot(dvc.spcoh1$empirical,dvc.spcoh1$timescales,type="pi",tsrange=c(3,7),xlab="Timescale",ylab="")
...
lines(dvc.res1$timescales,dvc.res1$emp.rank,lty=1,lwd=2,col="purple")
abline(h=0.95*nsurrogs,col="red",lty=2)
...
abun.dt1<-CleanData(cty.list$Abun[rownames(hunters.tmp),12:dim(cty.list$Hunters)[2]])
abun.wt1<-warray(abun.dt1$cleandat,times=1992:2016)
syncexpplot(resp.wmf=wmfwt(abun.wt1$wave.array),exp.sync = hunterabun.es$pred.wmf,1992:2016,hunterabun.es$timescales,ylab = "",xlab="Year",lwd=2)
...
mtext("A)",adj=0.05,line=-1.2,side=3,font=2)
wsurfplotTLA(CleanData(climindex$SummerPDO[1,])$cleandat,times=1981:2016,colorbar=T,type="power",xlab="",ylab="")
mtext("B)",adj=0.05,line=-1.2,side=3,font=2)
wsurfplotTLA(CleanData(climindex$WinterNAO[1,])$cleandat,times=1981:2016,colorbar=T,type="power",xlab="")
mtext("C)",adj=0.05,line=-1.2,side=3,font=2)
wsurfplotTLA(CleanData(climindex$SummerNAO[1,])$cleandat,times=1981:2016,colorbar=T,type="power",ylab="",xlab="")
mtext("D)",adj=0.05,font=2,line=-1.2,side=3)
wsurfplotTLA(CleanData(climindex$WinterMEI[1,])$cleandat,times=1981:2016,colorbar=T,type="power",xlab="Year",ylab="Timescale")
mtext("E)",adj=0.05,line=-1.2,side=3,font=2)
wsurfplotTLA(CleanData(climindex$SummerMEI[1,])$cleandat,times=1981:2016,colorbar=T,type="power",xlab="Year",ylab="Timescale")
...
mtext("A)",adj=0.05,line=-1.2,side=3,font=2)
wsurfplotTLA(CleanData(usda.list$Crashes[,!is.na(colSums(usda.list$Crashes))])$cleandat,times=1987:2016,colorbar=T,type="wmf",xlab="",ylab="")
mtext("B)",adj=0.05,line=-1.2,side=3,font=2)
wsurfplotTLA(CleanData(usda.list$Abun)$cleandat,times=1981:2016,colorbar=T,type="wpmf",siglevel = 0.999,xlab="")
mtext("C)",adj=0.05,line=-1.2,side=3,font=2)
wsurfplotTLA(CleanData(usda.list$Crashes[,!is.na(colSums(usda.list$Crashes))])$cleandat,times=1987:2016,colorbar=T,type="wpmf",siglevel = 0.999,xlab="",ylab="")
mtext("D)",adj=0.05,line=-1.2,side=3,font=2)
syncexpplot(resp.wmf=wmfwt(abun.wt$wave.array),exp.sync = model.es$exp.sync,times=1981:2016,abun.wt$timescales,lwd=2,ylab="")
mtext("E)",adj=0.05,line=-1.2,side=3,font=2)
syncexpplot(resp.wmf=wmfwt(dvc.wt$wave.array),exp.sync = dvcabun.es$pred.wmf,times=1987:2016,dvc.wt$timescales,lwd=2,ylab="")
mtext("F)",adj=0.05,line=-1.2,side=3,font=2)
dev.off()

#Plot the five year fluctuations
deer.wt<-warray(CleanData(cty.list$Abun)$cleandat,times=1981:2016)
deer.wt1<-apply(deer.wt$wave.array,c(2,3),mean,na.rm=T)
mei.wt<-wt(CleanData(win.mei.mat[1,])$cleandat,times=1981:2016)
