...
phases (as measured by $\left| \frac{1}{N}\sum_{i=1}^N \exp(2\pi u_i j)  \right|$, where $j$ is the imaginary unit 
...
We also provide technical background on the spatial wavelet coherence, taken from
@Sheppard2016_aphids and @Sheppard_wavmodel (but also available elsewhere) and reproduced 
...
is the magnitude of
\begin{equation}
\Pi_{\sigma}^{(01)}=\frac{1}{NT}\sum_{i=1}^N \sum_{t=1}^T w^{(0)}_{i,\sigma}(t) \overline{w^{(0)}_{i,\sigma}(t)},\label{eq:spatwavcoh}
\end{equation}
\noindent where the $w^{(k)}_{i,\sigma}(t)$ are the power-normalized wavelet transforms of the
$x_i^{(k)}(t)$ (see (\ref{eq:pownormwt}) above). But the complex number 
$w^{(0)}_{i,\sigma}(t) \overline{w^{(0)}_{i,\sigma}(t)}$ has phase equal to the phase difference 
between $w^{(0)}_{i,\sigma}(t)$ and $w^{(0)}_{i,\sigma}(t)$, so the sum over $i$ and $t$ in 
(\ref{eq:spatwavcoh}) will be large of this phase difference is consistent over space and time,
and will be small otherwise. The wavelet components $w^{(0)}_{i,\sigma}(t)$ and $w^{(0)}_{i,\sigma}(t)$
can also have varying magnitudes, and the spatial wavelet coherence will be larger still if 
there are correlations in how these amplitudes fluctuate through time. When $N=1$, the spatial
wavelet coherence is the wavelet coherence, a standard method [@Torrence1998; @addison2002].
The measure also relates to the phase coherence of a single pair of transforms, a measure 
which has been used previously in ecology [@Grenfell2001; @Viboud2006]. The phase of 
$\Pi_\sigma^{(01)}$, which we report in our tables, is the average phase difference between 
...
$x_i^{(0)}(t)$ and $x_i^{(1)}(t)$ regardless of whether they are in phase, or phase shifted.

<!--***DAN: made it to here, end of 2020 04 23. Continue tomorrow with significance.-->
