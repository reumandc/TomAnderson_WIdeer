...
```{r echo=FALSE, results="hide", cache=TRUE, fig.show="hide", cache.extra=list(mtime("Data/abunsurrsum.csv"),mtime("dvcsurrsum.csv"),                                                                                mtime("Code/FourierAnalysesDeer.R"),mtime("Code/FourierAnalysesDVCs.R"))}
...
and define $\gamma=q_{ 0}+q_{ 1}\mu+\cdots+q_{m}\mu^{m}$ and
...
the timeseries being transformed. Convolutions for which a substantial portion of the wavelet 
...
\textbf{The wavelet mean field and wavelet phasor mean field, technical background.} Technical background on the wavelet mean field was taken from 
...
are identical to the phases of the $W_{i,\sigma}(t)$, and equal the phase of oscillation
...
\textbf{Spatial wavelet coherence and wavelet coherence, technical background.} Technical background on the spatial wavelet coherence was taken from
@Sheppard2016_aphids and @Sheppard_wavmodel (but is also available elsewhere, e.g, @Prichard1994), and is reproduced 
...
there are correlations in how these amplitudes fluctuate. The aptial wavelet coherence 
...
\textbf{Wavelet Moran theorem, fractions of synchrony explained, and predicted synchrony.} We calculated 
...
\textbf{Wavelet Moran theorem, fractions of synchrony explained, and predicted synchrony, technical background.}
Technical background on the wavelet Moran theorem and related ideas was taken from
...
$|\Pi^{(01)}_{\sigma}||r^{(\text{\emph{wmf}},1)}_{\sigma}(t)|$. This is similar to (\ref{eq:Moranmain1}),
...
\begin{equation}
q(\sigma)=\frac{|\Pi^{(01)}_{\sigma}|^2\frac{1}{T}\sum_{t=1}^{T} |r^{(\text{\emph{wmf}},1)}_{\sigma}(t)|^2}{\frac{1}{T}\sum_{t=1}^{T}|r^{(\text{\emph{wmf}},0)}_{\sigma}(t)|^2}.\label{eq:syncexpl}
\end{equation}
\noindent This was averaged over a timescale band of interest.  

\textbf{Fraction of deer synchrony explained by winter MEI, technical methods.} Fractions of deer abundance synchrony explained by snow depth and fractions of DVC synchrony
explain by deer, and related results, were presented in the main text, but we did not there present results
on fractions of deer abundance synchrony explained by winter MEI, even though deer abundance and winter MEI
...
technical description of 1) why applying the wavelet Moran theorem to MEI and deer abundance 
can produce overfitted models and inflated estimates of fractions of deer synchrony explained
by MEI; and 2) steps we took to try to quantify the degree of inflation. Results of the
procedures described here are in SI section \ref{SI:results}.

<!--***DAN: once this is all written, go through and make sure the sub sections of the SI Methods and 
SI Results are A) in the same order, and B) in the order in which they are cited in the main text, to make
everything easier to find. The easiest way to do this is probably to just read the main text and follow
all the links. While you are at it, read and edit all the SI again - I have a feeling it needs one 
more round of that.-->

It is possible to apply the formalism of the wavelet Moran theorem to deer abundance data and 
winter MEI, even though doing so results in complications, as we will explain. We can thus write 
...
time series and the $w^{(1)}_{i,\sigma}(t)$ are the power-normalized wavelet transforms of winter 
MEI and therefore are the same for all locations, $i$. The theorem then provides a decomposition 
...
explained by winter MEI is just $|\Pi^{(01)}_{\sigma}|^2$. This is the quantity we statistically tested to 
...
supporting the claim that winter MEI explains a significant portion of the synchrony in deer abundance.
...
that there is probably a real association between winter MEI and deer fluctuations, bias may still manifest in the 
form of a spurious association between MEI and additional local variability in deer abundance that is actually 
independent of MEI, exagerating the apparent coherence between deer and MEI.  So the lack of spatial resolution in the 
winter MEI index means, in summary, that the estimated coherence, $|\Pi^{(01)}_{\sigma}|$ between deer abundance and MEI
(and therefore the deer synchrony, $|\Pi^{(01)}_{\sigma}|^2$, explained explained by MEI), was significant, but may be inflated: we know MEI-related Moran effects probably helped cause
deer abundance synchrony, but we do not know to what extent. Naive application of the wavelet
Moran theorem suggests that winter MEI explains $X\%$ of deer abundance synchrony 
on $3-7$-year timescales and $Y\%$ on $4-7$-year timescales. 
<!--***DAN: Tom, can you please fill these values in? I forgot what they are, 
and they have been
deleted from Table 1. These would be for the model with sole predictor MEI. Please 
use autolinks instead of typing in the answer.-->
The goal now is to estimate
to what extent these values are inflated.

<!--Description of procedure for estimating inflation-->
To assess the levels of inflation in MEI-deer coherence estimates and therefore in estimates of amount of 
deer synchrony explained by
MEI, we performed simulation exercises with artificial data chosen to mimic properties of the real time-series, but 
...
We first fitted an AR(2) model to the $36$-year winter MEI time series we used for our other analyses, obtaining
...
time steps to eliminate possible transient dynamics. We then used the resulting simulated winter MEI stand-in time series
...
transforms, $w^{(1)}_{i,\sigma}(t)$, of the MEI, which were the same for all $i$. For each simulated set of deer
...
paragraph, and compared these quantities to values of the same statistics computed for the actual winter 
MEI and deer abundance time series in order to find the values of $\sqrt{v}$ and $c$ for which agreement was good.

The three quantities were as follows. The first quantity was the spatial synchrony, measured using a 
...
were averaged over $3-7$-year timescales. The second quantity was the fraction of wavelet power, on average
over the $3-7$-year band, explained by the model $w^{(0)}_{i,\sigma}(t)=\Pi^{(01)}_{\sigma} w^{(1)}_{i,\sigma}(t)+d_{i,\sigma}(t)$. The denominator of this quantity was computed by:
1) computing, for each $\sigma$ and $i$, the average of $|w^{(0)}_{i,\sigma}(t)|^2$ over all available times;
2) then averaging the results of the first step over locations, $i$, and then over timescales
$\sigma$ in the $3-7$-year band. This is an average wavelet power for deer. The numerator was obtained
in the same way, but starting from $|\Pi^{(01)}_{\sigma} w^{(1)}_{i,\sigma}(t)|^2$ instead of
from $|w^{(0)}_{i,\sigma}(t)|^2$. The third quantity was the spatial synchrony, again measured
...
so this is an average of the squared spatial wavelet coherence. For the real data, these three quantities were,
respectively, $0.530557437095931$, $0.418327023890617$, and $0.364288221245834$, and we denote these values
by $A_0$, $B_0$, and $C_0$, respectively. We denote by $A(\sqrt{v},c)$ the median, over $500$ simulations,
of the first quantity described above for the simulations. Likewise $B(\sqrt{v},c)$ and $C(\sqrt{v},c)$ 
denote medians over $500$ simulatuions of the second and third quantities above. 
...
\textbf{Fourier methods.} Although our wavelet methods were most appropriate for initial discovery
of our results, many of our
results can also be demonstrated, \emph{post hoc}, using Fourier methods. So we here describe Fourier methods we
applied, which support our wavelet results. 

After discovering (Fig. \ref{MT-Fig3_DeerSync}A) that synchrony in deer is primarily at $3-7$-year timescales, to 
test whether the theory presented above is reflected in our deer data, it suffices to show
that spectral power in the $3-7$-year timescale band was much greater for the total deer time series 
...
generating those surrogates are described above). A $p$-value result for the test of the null hypothesis that
...
research goals, autoregressive surrogates are fairly common and have been used for significance testing
spectral quantities. AR(1) models have no spectral peak, so they have been used to test
for the significance of spectral peaks. Therefore, we also tested our 
...
Criterion (AIC). Recall that lower AIC values indicate better model-data correspondence. 
...
If our theory is reflected in deer data, then periodicity at $3-7$-year timescales should be stronger 
for the state-total deer time series than it is for the county-level deer time series, so we tested this. 
...
means that $3-7$-year spectral power is also influenced by the amount of low-frequency power.  

# Supplementary results \label{SI:results}

\textbf{Fraction of deer synchrony explained by winter MEI, county data, technical results.} 

<!--Qs for L and his answers as of 2020 05 12:
1) In your notes you sent, and which we saved on google drive, it says: " We considerd the median results to be a good fit if the sum squared fractional deviation of the three median results was found less than 0.05. "
BUT, in the table, first row, it says: "sum squared closeness of fit (best 6, $<$0.005)" and the values are less than 0.005. So is it 0.05 or 0.005, or did you mean two different things here? 
The value 0.05 is a typo, 0.005 is correct

Also, it says "best 6" but there are 8 numbers given, can I assume these were the best 8 and that these were the only ones under the cutoff you used (0.005 or 0.05)?
Relic from an earlier version, there were 8 good fits in this run.

2) In the table it says 1000 sims were done, but in the text it says 500. Which?
Relic from an earlier version, there were 500 done.

3) In the table, is "strength of local noises" your parameter eps? I.e., the standard deviation, not the variance of the noise.
4) In the table, is "correlation of local noises" meant to be the value of c, which is not a correlation but rather a covariance? A correlation would be c/eps^2

I defined a covariance matrix with ones on the diagonal, equal to c everywhere else.   So this is also a correlation matrix.   I generated deer noise with this covariance matrix using chol.m.   I multiplied the noise by strength eps when simulating the AR process.
***DAN: I fixed this in the Methods

5) Do the three lines "Power closeness of fit", "Synchrony predicted closeness of fit", and "Synchrony found closeness of fit" correspond to A, B and C? 

Yes.

Are the values here A(eps,c)-A_0/A_0 (rather than the absolute value of that), and that's why some are negative?

Yes.

6) In your email sent 5/5, you sent 8 numbers, and I am assuming these should replace the last row of the table. Is that correct? The second and third rows of the table should also be replaced, I think, to make all the results consistent. In order to avoid having anything lost in translation, can you please just send a new, updated version of the table, or replace the one in google drive with a new one?

there are going to be some incosistencies in this table because of the way it is cobbled together from two different runs.   I am starting to think that I should do a clean repeat of the exercise and make a whole new table.
-->



<!--***DAN: Lawrence is re-running over the weekend and will send a new versions of this-->
\begin{table}[h!]
\begin{center}
...
sum squared closeness of fit (best 6, $<$0.005) &
0.0040 0.0036 0.0043 0.0008 0.0026 0.0008 0.0020 0.0043\\

Actual (10000 yr) fraction of sync explained &
0.6316 0.6175 0.5845 0.5704 0.6005 0.5426 0.5345 0.4777\\

Synchrony explained (median of 1000 instances, 36 years of data) &
0.7592 0.7421 0.7353 0.7109 0.7185 0.7029 0.6953 0.6626\\
Wavelet power explained (median of 1000 instances, 36 years of data) &
0.4195 0.4225 0.4289 0.4097 0.4251 0.4079 0.4026 0.3971\\
Strength of local noises &
1.6000 1.6000 1.6000 1.7333 1.6000 1.7333 1.7333 1.8776\\

Correlation of local noises &
0.3000 0.3250 0.3500 0.3500 0.3750 0.3750 0.4000 0.4250\\

Power closeness of fit &
0.0028 0.0100 0.0254 -0.0205 0.0162 -0.0249 -0.0375 -0.0507\\

Synchrony predicted closeness of fit &
0.0308 0.0559 0.0596 0.0039 0.0433 0.0072 -0.0104 -0.0186\\

Synchrony found closeness of fit &
-0.0551 -0.0193 0.0074 -0.0186 0.0217 0.0113 0.0230 0.0372\\

RMS closeness of fit &
0.0365 0.0346 0.0377 0.0161 0.0295 0.0163 0.0261 0.0379\\

Synchrony explained inflation factor &
1.2021 1.2018 1.2580 1.2463 1.1966 1.2954 1.3010 1.3873\\
...
<!--Fourier results-->
\textbf{Fourier results.} Total spectral power in the $3-7$-year timescale band for detrended state-total
...
Fig. \ref{FigS_SpectPow3to7Fig5Surrogs}). We nevertheless provide these 
alternative analyses to demonstrate the robustness of our conclusions.
...
(see SI section \ref{SI:methods} for methods). This was a test of whether periodicity on these
...
Overall, the fourier results support the conclusions that:
1) there was a meaningful spectral peak in the $3-7$-year timescale band for the state-total deer time 
series for WI, and it was substantially caused by synchrony; and
2) the spectral peak in the $3-7$-year band is substantially greater for the state-total deer time series than 
...
for all of the surrogate time series of Fig.  \ref{MT-Fig3_DeerSync}C, indicating that synchrony in the $3-7$-year 
...
\caption[Phase differences between variables]{Phase differences between variables as a function of timescale for the main significant relationships of main text Table \ref{MT-Tab1_CountyCoherenceSummary}, county-level data: A) winter MEI ($\text{MEI}_{\text{w}}$) 
...
\begin{figure}[!h]
\includegraphics[height=.8\textheight]{Results/PedagogEmpFig.png}
\caption[Comparison of theoretical to empirical results]{Raw time series of predictors variables (A-C), response variables (D-F), wavelet mean field magnitudes (G-I), and state-total time series (J-L) for theory (left column), deer (middle column) and DVCs (right column). Horizontal and vertical axes differ between left and right panels because time series were shorter for DVCs. \label{PedagogEmp_Fig}}
\end{figure}
...
population time series for WI (see SI section \ref{SI:methods}), whereas the histogram shows values of the same 
...
are at the frequencies corresponding to timescales of 3 and 7 
years. At all the frequencies in that range, the spectrum of the data was above the spectra of all the 
surrogates. Spectra are identical sufficiently far outside the $3-7$-year timescale band because Fig. 
