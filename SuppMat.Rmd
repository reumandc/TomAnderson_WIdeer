...
\subsection{ Fourier and correlation methods}\label{SI:methods:Fouriermethods} 
\noindent Although our wavelet methods were most appropriate for initial discovery
of our results, many of our
results can also be supported, \emph{post hoc}, using Fourier and correlation methods. We here 
describe the Fourier and correlation methods we applied to further test our theory and support our wavelet results. 

After discovering (Fig. \ref{MT-Fig3_DeerSync}A) that synchrony in deer is primarily at $3-7$-year timescales, to 
test whether the theory presented above (SI sections \ref{SI:theory} and \ref{sect:mainproof}) is reflected in our deer data, 
...
the state-total deer time series was significant compared to AR(1) surrogates, 
for completeness. But we emphasize that we consider 
...
To provide still more evidence that state-total deer populations in WI were indeed cyclic, 
with periodicity in the $3-7$-year range and a statistical meaningful spectral peak within that timescale range, 
we used maximum likelihood to fit autoregressive models of different orders to 
