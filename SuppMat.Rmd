...
#Run analysis on deer abundance and DVCs using county and district data
set.seed(101)
system.time(source("Code/DeerCoherence_wsyn.R"))
```

<!---generate plots--->
<!---QC_Tests/test_plots.R produces code to provide roughly identical plots to those used in the paper, as a means of checking that the manuscript versions accurately reproduce the correct output.-->
...
($\pm 20\%$ error rates). 
<!--***DAN: It was Robert that told us this, and he is a coathor, so perhaps we should take out the personal communication parens? It would be best if we could get a citation for this, even a white paper, if there is one and we can fit it in. If we leave it as "personal communication" then that begs the question "personal communicccation with whom?" The WIDNR is not a person.--><!--TOM:Yeah we could probably take it out, which I did. Science doesn't permit references to unpublished things, so we couldn't do (...;Stenglein [the woman who did that analysis], unpublished data)-->
...
<!--***DAN: I added the above sentence, Tom, based on a comment of Roberts that had been inserted as a marginal note in the Word doc that this Rmd replaces. Please check. If we keep the "personal communication" paren above then we should also insert one here.--><!--TOM: I took the one out above, so should be fine here-->
Nevertheless, 
to ensure our results were not biased, we analyzed 
synchrony on multiple spatial scales. USDA district data were 
...
Fig. \ref{FigS6_WMFAndPredictionsDistricts}), suggesting results for counties were not 
...
variances were also standardized to 1. These steps were done via the `cleandat` function in the `wsyn` package for the R programming language, available at `www.github.com/reumandc/wsyn`.
<!--***DAN: When we get closer to the end, add version info, and if it's on CRAN by then cite the CRAN version with its version info-->
...
some biological fields, well established and widely applied; they are described in 
...
the timescale considered (Figs \ref{MT-Fig1_example}, \ref{PedagogFigWMF}). 
...
Fig. \ref{FigS7_WMFsAndWPMFsAncillary}. Wavelet mean fields were computed using the
function `wmf` and wavelet phasor mean fields were computed using the function `wpmf`. Both of these functions are in the `wsyn` package for the R programming language.

All plots based on wavelet transforms are "scalloped", i.e., values corresponding to longer timescales and times closer to the start or end of available data are not plotted. This is because the wavelet transform is based on convolutions of wavelet functions with the timeseries being transformed; convolutions for which a sunstantial portion of the wavelet function overhangs the edge of the data are unreliable. "Scalloping" wavelet plots is standard practice.
...
technique. This technique is a standard tool in some biological fields, well established and widely applied; it is described in detail 
...
<!--***DAN: Tom, there is an nsurrogs variable defined near the top of this document that is currently 10000. Do you plan to rerun with 100000 later? Either way, unless I am mistaken about the purpose of that variable, please turn the above N=100000 into a dynamic link so it updates automatically and displays the correct number of surrogates that were actually used on the latest run. I made the change but I am flagging it here to make sure you see it so that in case it causes problems somehow that I'm not aware of you can do something about it. Delete this comment if the change makese sense to you.
Also, and importantly, when you rerun with 100000 (if indeed we do - not clear to me it's strictly necessary) we will have to re-check that all statements are consistent with the new results, because all the results will change. They should not change much, but they will change. Will have to review for this.--><!--TOM: Yeah, making it dynamic makes sense, jsut something I hadn't caught yet since I hadn't edited the text in a while. I dropped down the surrogate number just to decrease run time when I was working out all the kinks of knitting- we can raise it back up. -->
...
<!--***DAN: Tom, see previous comment. I also changed the line right above this to be an automated link--><!--TOM: looks good-->
...
(Fig. \ref{FigS3_PhaseDiffs}). Spatial wavelet coherences were computed and results were processed using the `coh`, `bandtest`, `plotrank`, `plotphase` and related functions in the `wsyn` package for the R programming language. The fast algorithm of [@Sheppard2017_surrogates] for assessing the statistical significance of spatial wavelet coherence was used.

In contrast to correlation-based approaches that 
assume in-phase relationships, spatial wavelet coherence 
...
As with correlations, causal relationships between variables are not directly revealed by the spatial wavelet coherence. But biological arguments 
...
statewide total DVCs. Functions in the `wsyn` package were used, and can compute ordinary wavelet coherence as well as spatial wavelet coherence.
...
<!--***DAN: Tom, you had attached a bunch of marginal comments to the version of this para in the Word doc, and I tried to act on those as best I could. Please revise the result-->
...
We have adopted an open science approach throughout the project. As indicated in the sections above on individual methods, all computations were done using functions from the `wsyn` package for the R programming language, the source code of which is available from `www.github.com/reumandc/wsyn`. The package can be installed into R via `devtools::install_github(repo="reumandc/wsyn")`.
<!--***DAN: Dan, change if we get it on cran before submission-->
The `wsyn` package includes a vignette documenting how to use the functions implemented there, and giving examples. All data and code producing this paper are archived at X, including the complete revision history of the process that led to the presented analyses.
<!--DAN: update when we get close-->
The suite of analyses leading to this paper can be re-run and the paper re-compiled at the touch off a button via the codes in that repository - see the Readme.md file in the repository for instructions.
<!--***DAN: Dan, you need such a readme, add it after you split the paper into its own repo. This will take some work as you'll have to say something about dependencies.-->
...
(Fig. \ref{FigS6_WMFAndPredictionsDistricts}A,C).

Deer abundance was significantly related to $\text{MEI}_{\text{w}}$ and $\text{MEI}_{\text{s}}$, 
...
reduced at the larger district level because reduced fractions of deer and cars dispersal from one spatial unit to another for these larger spatial units. 
Patterns of synchrony in DVCs predicted by synchrony in deer looked similar to actual patterns of synchrony in DVCs (Fig. \ref{FigS6_WMFAndPredictionsDistricts}F). 
...
<!--***DAN: commented this out because if it is here we should also look at the 3-5 range and maybe over other sub-ranges of 3-7. Tom, please delete or add a slightly more complete analysis-->
<!-- ***TOM: Deleted it, didn't really seem necessary or add much. -->
$\text{MEI}_{\text{s}}$, $\text{MEI}_{\text{w}}$ and $\text{PDO}_{\text{w}}$ were significantly 
...
numeric callouts right at the very end. If you could figure out a way to automate those, it would be great, but I don't think its possible because the below is latex code.-->
...
<!--***DAN: Tom, can you please remove the blue cross-hatching on panel G, as there is no corresponding mean phase given for that range (and there should not be a mean phase given since the coherence was not significant over that range).--><!--TOM: Removed it, but my code leaves vertical dashed lines at 3-7. If you think those need to go as well- the alternative would be to just add text saying we tested that but wasn't significant.-->

\newpage

<!--***DAN: To get the references in the below caption to work,
I had to do them manually. Right now I am using author-date, but it needs to be changed to
numeric callouts right at the very end.-->
...
actual wavelet mean field magnitude of deer abundance, similar to the one shown in Fig. \ref{MT-Fig2_DeerSync}A). Slight differences exist here vs \ref{MT-Fig2_DeerSync}A) because of different spatiotemporal extents of the data used in hunter analyses. In any case, agreement is poor, 
giving a 
...
They do not (they are not even particularly close!). What's going on?--><!--TOM: Two things: first, I was plotting HUNTER abundance as the contours, not deer abundance- so that's why they looked so bad was because it was the wrong thing to be plotted. Second, it won't look identical to 2A because of the differences in spatial extent (only 53 counties could be used for the hunter analysis. It also is 10 years shorter so truncates things as a bit in the temporal dimension as well. I tried to specify that above. I did make a wmf plot of the deer abundance truncated to the hunter data set dimensions and they do now visually match- this way seems more appropriate so that everything lines up, but let me know if you disagree.)-->
...
\caption{Wavelet mean fields (left column) and wavelet phasor mean fields (right column) of winter Tmin (A, B), Tmax (C, D), snow depth (E, F) and precipitation (G, H) using county-level data. Colors and contours as in Fig \ref{MT-Fig2_DeerSync}A,B. \label{FigS7_WMFsAndWPMFsAncillary}}
\end{figure}
...
knitr::kable(TableS1,
             ...
             caption = "Summary of spatial wavelet coherence tests and related results using county-level data. $P$-values and mean phases are from tests of spatial wavelet coherence (*Methods*) of the variables in columns 1 and 2 over the indicated timescale band. Mean phases over the timescale band (units of $\\pi$ radians) are provided only when meaningful, i.e., when spatial wavelet coherence was significant, $P < 0.05$, or marginally significant. A positive mean phase indicates the variable in the column *Response* is leading, and a negative mean phase indicates the variable in the column *Predictor* is leading. The *Synchrony explained* and *Average cross terms*  columns (which give percentages) are based on wavelet linear regressions (*Methods*) of the *Response* transforms against the *Predictor* transforms. When cross terms are low, synchrony explained is interpretable as the approximate percentage of syncrony in the response explainable as due to the synchrony in the predictor. They were low enough except for tests involving DVCs, for which USDA district-level results were more reliable (main text and Table \\ref{TabS4_SummaryDistrict}). Cross-terms are zero for test involving climate indices because for those tests the same values were used for all locations (*Methods*). All values were rounded to display four digits. \\label{TabS1_CountyCoherenceSummary}",
             booktabs=T)
```

<!--***DAN: Tom, please review this caption carefully, as I have edited it heavily. I looked through the code to tryto make sure I was not introducing any errors in descriptions of what was done, but please be sure yourself, too, to be on the safe side. I emailed you about this and I have also taken your response into account, but another check from you would be useful.-->
...
<!--***DAN: Tom, in the first two columns you get labels like "WinterNAO" instead of "Winter NAO" (with a space). Can you please fix that? Not just for Winter NAO but for all entries in the first two columns. Like wise please fix the same problem as it occurs in Tables S3 and S5.-->
...
knitr::kable(TableS4,
             ...
             digits=4,
             col.names=c("Predictor","Response","Timescale", "P-value","Mean phase ($\\bar{\\theta}$)","Synchrony explained","Average cross terms"),
             caption = "Summary of spatial wavelet coherence tests and related results using USDA district-level data (*Methods*). $P$-values and mean phases are from tests of spatial wavelet coherence (*Methods*) of the variables in columns 1 and 2 over the indicated timescale band. Mean phases over the timescale band (units of $\\pi$ radians) are provided only when meaningful, i.e., when spatial wavelet coherence was significant, $P < 0.05$, or marginally significant. A positive mean phase indicates the variable in the column *Response* is leading, and a negative mean phase indicates the variable in the column *Predictor* is leading. The *Synchrony explained* and *Average cross terms*  columns (which give percentages) are based on wavelet linear regressions (*Methods*) of the *Response* transforms against the *Predictor* transforms. When cross terms are low, synchrony explained is interpretable as the approximate percentage of syncrony in the response explainable as due to the synchrony in the predictor. They were low enough for all results in this table. Cross-terms are zero for test involving climate indices because for those tests the same values were used for all locations (*Methods*). All values were rounded to display four digits. \\label{TabS4_SummaryDistrict}",
             booktabs=T)
```
