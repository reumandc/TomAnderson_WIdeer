...
<!--***DAN: I think I dont need this anymore, keeping it just in case:
We will later use the
notation $\epsilon_{ij}$ for the bivariate process $(\epsilon_i,\epsilon_j)$ and
$w_{ij}$ for the bivariate process $(w_i,w_j)$.
The spectral matrices of $\epsilon_{ij}$ and $w_{ij}$ are therefore denoted,
respectively, $S_{\epsilon_{ij},\epsilon_{ij}}(f)$ and $S_{w_{ij}w_{ij}}(f)$.-->
...
<!--***DAN: currently this section ref does not work, fix-->
...
Theorem \ref{thm:lem} in section X provides the spectra
<!--***DAN: fix this section reference-->
...
<!--***DAN: explain noise for example 3-->
...
<!--***DAN: Tom, you need to be consistent with how you do your section headers. Sometimes
you use the ## approach and sometimes you use the \textbf approach. I suggest we always 
use the #, ##, ### approach because I know how to label those and refer to them with
hyperlinks-->
...
\caption{Results for theoretical example 1. See X for a full description of the
...
\end{figure}
<!--***DAN: Replace X with a reference to the correct section once sections are organized properly-->

\begin{figure}[!h]
\includegraphics[width=\textwidth]{Results/TheoryExample2.png}
\caption{Results for theoretical example 2. See X for a full description of the
...
\caption{Results for theoretical example 3. See X for a full description of the
