...
Our wavelet approach, based on the continuous Morlet wavelet transform, can be derived from Fourier methods 
and can be defined in terms of Fourier transforms. For instance, 
because the continuous Morlet wavelet transform at a given timescale is defined as the convolution of a wavelet function with
that scale with the time series, it can also be defined (by the convolution theorem) simply as the inverse Fourier transform of the product of the 
Fourier transform of the wavelet function with the Fourier transform of the time series. 
Because all our wavelet approaches are based on the continuous Morlet wavelet transform, and because we have just argued that 
that transform can be defined in terms of Fourier transforms, in fact all our wavelet methods actually \emph{are} Fourier methods.
Although there may appear to be a mismatch between a Fourier theory and a wavelet empirical analysis, the fact that wavelets actually 
are Fourier methods would tend to suggest this is only superficially a mismatch. 
Not only can the continuous Morlet wavelet transform be formally defined in terms of Fourier analyses, but wavelet quantities can also be interpreted
in the same way as Fourier quantities. The wavelet power (an integral of the square magnitude of the wavelet transform)
can be interpreted in the same way as the power spectrum. An average through time of the wavelet phase difference between two time series
can be interpreted in the same way as the phase of the cross spectrum of the time series.
The average of a wavelet mean field over time is interpretable in the same way as the average of all pairwise
cospectra of a set of time series. 
Actually wavelet analysis is really nothing more than time-localized Fourier analysis. Theoretical results from a Fourier 
approach can certainly be compared to empirical results using wavelets.
...
complexities of real data, to discover whether our theoretical results 
are also substantially reflected in real data. Wavelet methods have frequently been applied and shown to be very useful, recently, in work on
the timescale specificity of synchrony [@Sheppard2016_aphids;@Walter2017_geogsync;@Anderson2019_LO;@Sheppard_wavmodel;@Walter_etal_2019;@Walter_etal_InPress].
...
see a mismatch between our Fourier-based theory and our wavelet empirical results, we nevertheless still 
provide a complete set of Fourier-only data analyses as additional support for our empirical conclusions. 
...
of the polynomial $1-b_1 z - b_2 z^2 - \cdots - b_n z^n$ are assumed to have modulus greater than $1$.
