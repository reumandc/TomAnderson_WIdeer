...
\begin{equation}
T(\mathcal{B}) S_{ww} \overline{T(\mathcal{B})} = T(\mathcal{P}) S_{\epsilon\epsilon}
\overline{T(\mathcal{P})}
\end{equation}
\noindent where $T(\mathcal{B})$ and $T(\mathcal{P})$ 
are the transfer function matrices of the linear filters
$\mathcal{B}$ and $\mathcal{P}$, respectively (see, for instance,
...
knitr::kable(TableS1,
             ...
             caption = "$P$-values of wavelet coherence tests, and associated mean phase differences (units if $\\pi$ radians) for significant and marginally significant tests, between climate indices. Values are over $3-7$ year timescales. All values were rounded to display four digits. \\label{TabS1_PairwiseClimVarSpatCoh}",
             booktabs=T)
```
