...
<!--TOM: We need to switch this to wsyn, not sure how to do that-->
...
#devtools::install_github(repo="reumandc/wsyn",ref="AndersonDeer",force=FALSE))
...
library(wsyn,lib.loc=.libPaths(),quietly=T)
```
...
#Clean population data: end result are two lists of matrices called "cty.list" and "usda.list". Cty stands for county. All elements of cty.list are matrices, counties by year. Elements are named abun (abundance), crashes (DVCs) ...
#***TOM: pls add sufficient details of what the format is for cty.list and usda.list, enough so whoever reads this comment would then be able to use those data
#***TOM: get rid of the flag - just make it do both, no is as good a time as any to do that
#***TOM: make the code save these objects as .rds and say to here
...
system.time(source("Code/Deer_DataGeneration.R"))
  #list of files generated by deer_DataGeneration.R:
    #scale.flag == "usda": usda.list, includes data on abundance, DVCs and traffic
    #scale.flag == "county": cty.list, includes data on abundance, DVCs and traffic
    #scale.flag == "both": both the above

#clean climate data
#***TOM: similar comments to above 
system.time(source("Code/Climate_DataGeneration.R"))
#creates four lists of data: winter weather and climate indices each for counties and USDA districts
    #scale.flag == "usda": usda.list, includes data on abundance, DVCs and traffic
    #scale.flag == "county": cty.list, includes data on abundance, DVCs and traffic
    #scale.flag == "both": both the above
