...
<!--***DAN: Revisit the presentation of these examples to try to make it clearer
how they instantiate the ideas-->
\textbf{Example 1.} For theoretical example 1, we constrasted a case for which
...
\textbf{Example 2.} For theoretical example 2, we contrasted a case in which
...
\textbf{Example 3.} For theoretical example 3, we contrasted a case in 
...
at smaller spatial scales [@Millspaugh2009]. The prior criticism of the 
SAK model [@Millspaugh2009] may have been overstated, as more recent analyses 
within the WIDNR 
<!--***DAN: Tom, I added "within the WIDNR" because we have no reference for these
"more recent analyses". Can we get a reference, or something more concrete than
what we currently have? Even a white paper?--><!--***Tom: I emailed the person who did that analysis to find out-->
suggest the uncertainty in SAK estimates is substantially less 
($\pm 20\%$ error rates) than what was reported there. 
...
wavelet linear models we 
report, but could not reasonably have artefactually created them.
<!--***DAN: Tom, please make sure you are OK with the sentence above, 
which I added. SAK inaccuracies should have added noise to the data,
but could not really have been what was responsible for timescale-specific
synchrony, since SAK inaccuracies could not reasonably have been periodic 
in nature. Similarly, it seems highly unlikely that SAK inaccuracies could
have arteefactually produced the spatial wavelet coherence and wavelet linear
model results, for similar reasons.--><!--***Tom: Sounds good-->
...
<!--***DAN: Tom, please check and fix how this references comes out, which is wonky - the callout is 
Team (2018)! I also think we should be citing the package, here, not R - see my comment in the
main text--><!--***Tom: Changed to the package-->
...
<!--***DAN: Tom, please insert an formal citation to the package-->
...
abundance at $3-4$-year timescales (main text Table \ref{MT-Tab1_CountyCoherenceSummary}; see also
...
to deer synchrony over $4-7$-year timescales (main text 
Table \ref{MT-Tab1_CountyCoherenceSummary}, see also 
...
(main text Table \ref{MT-Tab1_CountyCoherenceSummary}). Snow depth did not explain $4-7$-year 
synchrony: spatial wavelet coherence between snow depth and deer abundance on those timescales was 
not significant (main text Table \ref{MT-Tab1_CountyCoherenceSummary}, 
...
\caption[Phase differences between variables]{Phase differences between variables as a function of timescale for the main significant relationships of main text
