...
<!--***DAN: fill in the stuff Lawrence did, methods-->
...
terms equal to $c$. We later also consider an AR(1)-type model (see below). For given values of the parameters $\sqrt{v}$ and $c$, we performed wavelet transforms and 
...
BUT, in the table, first row, it says: "sum squared closeness of fit (best 6, $<$0.005)" and the values are less than 0.005. So is it 0.05 or 0.005, or did you mean two different things here? Also, it says "best 6" but there are 8 numbers given, can I assume these were the best 8 and that these were the only ones under the cutoff you used (0.005 or 0.05)?
...
5) Do the three lines "Power closeness of fit", "Synchrony predicted closeness of fit", and "Synchrony found closeness of fit" correspond to A, B and C? Are the values here A(eps,c)-A_0/A_0 (rather than the absolute value of that), and that's why some are negative?
...
<!--***DAN: Tom, Lawrence gave this table as a latex table. Can you please convert it to the kable format
you have been using so everything works with the citations and so on? It should also be relocated below so 
it is with the other tables, and so that the number it gets and is cited as is in line with the order the
tables are cited.-->
\begin{table}[h!]
\begin{center}
