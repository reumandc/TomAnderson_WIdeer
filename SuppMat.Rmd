...
#Basically this is set up so if anything but the write-up changes, everything is rerun. Simple and safe 
#but may waste some CPU time
...
get the same thing.-->
<!---To do citations, insert all the citations from the main text up here, but have them be invisible and then the numbers should line up starting below. Then in the main text, have the citations from the supp mat at the end of the main text --->
...
<!--***DAN: Tom, the contours on F look to me like they might not be the contours of the colors that
are mapped on B. I cannot be sure (it is not so easy to visually do such a comparison in much detail),
but there are enough discrepancies so that I want to ask you to check whether the contours mapped on F
actually are the contours of the colors on B. The main discrepancy that I imagine might exist centers
on the red blob on B that is centered at 2005 and about timescale about 3.5 That red blob does not seem
to be well refelcted in the contours in F. In fact, F to me looks identical to Fig. 3C - I think you may
have plotted the county-level data here instead of the distrct data. Likewise, E looks identical to Fig. 
2C, so I think the error might affect that panel, too.-->
...
<!--***DAN: Tom, it is a bit awkward to put this info here. It could be in the caption (but then I cannot get the 
dynamic links to work), or in the table itself (by adding another column called "Timescales"). What do you think,
Tom? We could either leave it here (maybe awkward) or make a new column (might be a pain).-->
$^1$ For snow depth and deer abundance over $3-5$-year timescales, $P=$ `r round(snow_abun_pval3_5,4)`, $\bar{\theta}=$ `r round(snowabun_phase3_5,4)`, and synchrony explained is `r round(abun_snwd_syncexp3_5,4)`. For hunters and deer abundance over $2-2.5$-year timescales, $P=$ `r round(hunterpval2_2.5,3)`, $\bar{\theta}=$ `r round(hunterphase2_2.5,4)`, and synchrony explained is `r round(hunterabun_syncexp2_2.5,4)`.

<!--***DAN: Tom, can you please replace "WinterNAO" with "Winter NAO", and likewise add spaces for "WinterPDO", "WinterMEI", etc, in the above table?-->
...
knitr::kable(TableS2,
             ...
             format='pandoc',
             caption = "$P$-values of wavelet coherence between climate indices. Values inside parentheses indicate the mean phase difference between variables, over $3-7$ year timescales (only shown for pairs of variables that were both significantly coherent with deer abundance). \\label{TabS2_PairwiseClimVarSpatCoh}",
             booktabs=T)
```

<!--***DAN: The values in parens did not come out in the pdf. Can you fix, please, Tom.-->
<!--***DAN: Also, and very importantly, it used to say
"P-values of spatial coherence between..." but these are all wavelet coherences (or should be) not spatial 
coherences, since these are all single time series. I changed it, but Tom can you please go back to the code
and look and see whether you actually computed spatial coherences with spatially replicated time series
or wavelet coherences between single time series? Should be the latter. It may (or may not) turn out to make
a difference, I am not sure, but we should anyway do the appropriate thing. See also the point in the main text 
in the para that begins "Spatial wavelet coherence, wavelet regression techniques...", in which a value from 
this table is quoted (the one for Winter MEI and Winter PDO). I added a note there that it was a wavelet coherence
result (not a spatial coherence result), so it needs to be checked that it is.-->
