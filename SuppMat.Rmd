...
par(fig=c(ywd/tot.wd,
          ...
          (xht+pan.ht)/tot.ht),
    mai=c(0,0,0,0),mgp=c(3,0.75,0))
ylimits<-range(deerlog10specbrill,surrlog10specsbrill)
plot(freqs,deerlog10specbrill,type="l",lwd=2,ylim=ylimits)
for (counter in 1:(dim(surrlog10specsbrill)[1]))
{
  lines(freqs,surrlog10specsbrill[counter,],type="l",col="grey",lwd=.5)
}
lines(rep(1/3,2),ylimits,lty="dashed")
lines(rep(1/7,2),ylimits,lty="dashed")
mtext("Frequency",side=1,line=2,cex=1)
mtext("Log10 power spectrum",side=2,line=2,cex=1)
dev.off()
...
par(fig=c(ywd/tot.wd,
    ...
    mai=c(0,0,0,0),mgp=c(3,0.75,0))
ylimits<-range(deerlog10specbrill,surrlog10specsbrill)
plot(freqs,deerlog10specbrill,type="l",lwd=2,ylim=ylimits,xlab="",ylab="")
for (counter in 1:(dim(surrlog10specsbrill)[1]))
{
  lines(freqs,surrlog10specsbrill[counter,],type="l",col="grey",lwd=.5)
}
lines(freqs,deerlog10specbrill,type="l",lwd=2,ylim=ylimits)
lines(rep(1/3,2),ylimits,lty="dashed")
lines(rep(1/7,2),ylimits,lty="dashed")
qsurrspec<-apply(FUN=quantile,X=surrlog10specsbrill,MARGIN=2,prob=c(0.95,1-.05/sum(freqs>=1/7 & freqs<=1/3)))
lines(freqs,qsurrspec[1,],type="l",lty="dashed")
lines(freqs,qsurrspec[2,],type="l",lty="dotted") #Bonferroni corrected threshold
mtext("Frequency",side=1,line=2,cex=1)
mtext("Log10 power spectrum",side=2,line=2,cex=1)
dev.off()
...
lines(freqs,qsurrspec[1,],type="l",lty="dashed")
lines(freqs,qsurrspec[2,],type="l",lty="dotted") #Bonferroni corrected threshold, freqs in 3-7yr timescale band
lines(freqs,qsurrspec[3,],type="l",lty="dotted") #Bonferroni corrected threshold, all freqs
mtext("Frequency",side=1,line=2,cex=1)
mtext("Log10 power spectrum",side=2,line=2,cex=1)
dev.off()
...
<!--***DAN: Add a section here explaining the Fourier methods that were used to generate 
supporting results-->
\textbf{Fourier results, deer.} Although our wavelet methods were most appropriate for discovery
...
Because autoregressive surrogates are fairly commonly used, we also tested our 
results using AR(1) surrogates, for completeness. However, we emphasize that we consider 
autoregressive surrogates less appropriate for our research context because they do not 
...
used the resulting model to generate surrogate timeseries. Spectral power in the 
...
<!--***DAN: Add a section here with Fourier results-->
