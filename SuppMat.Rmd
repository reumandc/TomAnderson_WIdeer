...
We start by specifying the model we consider. 
...
which has the same variance as $w_{\text{tot}}(t)=\sum_{i=1}^N w_i(t)$.
...
(i.e., its timescale). Throughout, we use for units of $f$ cycles per interval between 
...
So the cospectrum, denoted $C_{\epsilon_{i}\epsilon_{j}}(f)$, is $\Rp(S_{\epsilon_{i}\epsilon_{j}}(f))$, 
...
the spectrum of $w_{\text{tot}}$ (which is the same as that of $x_{\text{tot}}$).
...
\label{prop:mainthm}
\end{thm}
The proof is in section \ref{sect:mainproof}.
...
the terms in each sum, but the following several examples show the intuition
typically is valid.
...
## Proof of the main theoretical result \label{sect:mainproof}
...
\caption{Results for theoretical example 1. See X.}\label{TheoryExample1}
\end{figure}
<!--***DAN: Replace X with a reference to the correct section once sections are organized properly-->

\begin{figure}[!h]
\includegraphics[width=\textwidth]{Results/TheoryExample1.png}
\caption{Results for theoretical example 2. See X.}\label{TheoryExample2}
\end{figure}
<!--***DAN: Replace X with a reference to the correct section once sections are organized properly-->

<!--***DAN: add here one fig for each additional theoretical example-->
