...
<!--Step 2, part 1: first steps - correlation analyses and correlation analyses with pre filtering.-->
...
<!--Step 2, part 2: fitting of an ARX model - ***DAN: seems likely to turn out to be unnecessary-->
```{r alt_Fourier_2_p2p1, echo=FALSE, results="hide", cache=TRUE, fig.show="hide", cache.extra=list(mtime("Code/Review2_Modelling.R"),mtime("Results/cty.list.rds"),mtime("Results/winter.clim.rds"),mtime("Results/climindex.rds"))}

source("Code/Review2_Modelling.R")
```
<!--***DAN: I also have some code which is the beginnings of testing the model which is fitted above. I also think it'll be unnecessary. I had
not yet invoked it in a chunk here when I realized it would probably be unnecessary.-->
...
\noindent After presenting our theory (main text and SI section \ref{SI:theory}), we used wavelet statistical 
tools in the main text to argue that the theory was substantiated by deer and DVC data from Wisconsin, i.e., that 
timescale-specific synchrony is responsible for cycles observed at large spatial scales in deer and DVC data. 
The overall logic was: 1) we showed that synchrony in both deer and DVCs is timescale specific,
...
The alternative Fourier approach we decribe here follows the same overall logic. The logical steps correspond to 
...
<!--***DAN: make sure to automate citations when you embed, also search generally for "REF" and "REFS"-->
...
We also judged if peaks in $\rho_w (f)$ represented significant spectral features by comparing $\rho_w (f)$ for real data to 
...
sampling location, $i$. Synchrony as reckoned using standard correlation and covariance measures 
...
was higher than would have been expected if synchrony was evenly distributed across frequencies. When 
...
been expected if synchrony was evenly distributed across frequencies. Thus this approach revealed 
timescale specificity in synchrony.

We also computed the value of a "peakiness" statistic for $\rho_w (f)$, as computed using data, to values of the same
...
a test of the null hypothesis that peakiness from data was no greater than would have been expected by
chance if synchrony were evenly distributed across frequencies. 
...
is a local weather variable, potentially taking different values in each patch. The combined process $\epsilon=(\epsilon^{(1)},\epsilon_1^{(2)},\epsilon_2^{(2)},\ldots,\epsilon_N^{(2)})$
...
for background on stochastic processes and linear filters. The first specific model will be applied, for instance, 
to consideration of how and to what extent synchrony in deer can be attributed to MEI. The $w_i$ will model
...
variables which influence the populations $w=(w_1,\ldots,w_N)$. The combined process $\epsilon=(\epsilon_1^{(1)},\ldots,\epsilon_N^{(1)},\epsilon_1^{(2)},\ldots,\epsilon_N^{(2)})$
...
$\mathcal{P}^{(1)}$ represents two different filters in these two equations) suggests the general framework
to follow. The second specific model will be applied, for instance, 
...
as special cases. Results proved for the general framework immediately apply to both
specific models.
...
\label{eq:contrib2}
\end{equation}
\noindent which is based only on the measured quantities $w$ and
...
We henceforth
denote this quantity by $\rho_w^{(\epsilon^{(1)})}(f)$. It can be compared to $\rho_w(f)$.
\end{thm}
\begin{proof}
We have
\begin{equation}
...
detrended, normalized, and variance-stabilized prior to application of the model), we need to first consider whether model 
...
or, seperately, to our DVC (as $w$) and deer (as $\epsilon^{(1)}$) time series. The reason our first and second specific models apply to
...
to our data does not hinge on $\epsilon^{(2)}$. Likewise the application of our theory to our data does not hinge on
what parameters $b_1,\ldots,b_n$ and $p_0^{(1)},\ldots,p_m^{(1)}$ turn out to be the right ones to render our models 
adequate for our data: in the end the only quantities our Fourier Moran approach makes use of are the spectral matrix
...
and $\mathcal{P}^{(2)}$ taken as the identity filter. Since pre-treatment of the data makes it reasonable to assume 
...
being MEI, and our assumption that $S_{\epsilon^{(1)} \epsilon^{(1)}}$ is invertible for $\epsilon^{(1)}$
equal to snow depth or deer data from subsets of sampling locations (see SI section \ref{numerics_model2} for the reasons
subsets were used) and found these assumptions to be supported. We verified (see below) that for reasonable choices of
$b_1,\ldots,b_n$ the complex roots of the polynomial $1-b_1 z- \cdots - b_n z^n$ all have modulus greater than 1, as assumed.

Although the above logic is sound, we carried out some elementary cross correlation analyses and lagged regression analyses
to further support our modelling choices with some concrete comparisons. Average (across locations)
...
Regressions were also carried out of deer against lagged deer, and present and lagged snow depth and MEI (lags up to 3 years). 
Results are in SI section \ref{Crosscorr_and_regression}.

\subsubsection{Numeric and significance procedures associated with the first specific model}\label{numerics_model1}

\noindent We applied the first specific model of the previous section to deer populations (as $w$) and MEI (as $\epsilon^{(1)}$). 
Given data, and as specified in the previous section, we plotted estimates of $\rho_w^{(\epsilon^{(1)})}$ 
...
these procedures and methods we used to assess statistical significance of results.
...
$\rho_{w,\Omega}(f)$ are not unduly focussed on short-range synchrony.
...
\subsection{Use of Fourier approaches for the theory and wavelets for the empirical analysis}\label{SI:methods:methodsmismatch}

<!--***DAN: I just parked this material here, I have not revisited or adapted it yet. 2020 09 10.-->
\noindent Fourier and
...
theoretical and empirical analyses. Many Fourier analyses make more structural assumptions 
...
mathematical analysis were possible. We then used
...
# Alternative analysis: results \label{sect:alt_results}
...
to be highly statistically significant in comparison to phase-randomized surrogates (Fig. \ref{ComprehensiveFourier_SynchronyIsTimescaleSpecific_phase}).
See SI section \ref{sect:FourierTSSync} for details on phase-randomized surrogates.
...
timescales (Fig. \ref{ComprehensiveFourier_SynchronyIsTimescaleSpecific_permute}). This demonstrates that deer
...
$`r round(100-100*pval_deer_peakiness,2)`$% of the random-permutation surrogates we used. Thus the null 
...
frequencies). Our "peakiness" statistic (SI section \ref{sect:FourierTSSync}) took a larger value for DVC data than it did for
$`r round(100-100*pval_dvcs_peakiness,2)`$% of random-permutation surrogates, demonstrating that the peak in 
$\rho_w(f)$ in the $3-7$-year timescale range was a meaningful spectral feature 
($p = `r round(pval_dvcs_peakiness,4)`$, based on $10000$ surrogates).
...
second specific models were appropriate for our data, we carried out the elementary regression and 
...
<!--***DAN: insert a specific referece to a wavelet result here-->
...
showed the strongest relationship between deer and snow depth.
<!--***DAN: insert a specific referece to a wavelet result here-->
...
original data, again indicating a relationship between deer and snow depth, and supporting the 
wavelet results. 
...
remove all variation outside the $3-4$-year range (both deer and snow depth time series were 
                                                   filtered). This was done because the relationships between deer and snow depth revealed by wavelet 
analyses were primarily on $3-4$-year timescales. The $p$-value result using the un-lagged correlation
statistic described above was `r round(cor_pvals_deer_snow["p_onetailed_stat1_filt"],4)`, and the result using the
lagged statistic was `r round(cor_pvals_deer_snow["p_onetailed_stat4_filt"],4)`, again supporting our wavelet results.

We also carried out similar analyses for deer and MEI, though we did not do an unlagged correlation analysis
because wavelet analyses indicated a quarter-phase relationship between deer and MEI, 
<!--***DAN: insert a specific referece to a wavelet result here-->
  and quarter-phase relationships are associated with correlations of zero. We averaged lagged correlations between 
deer and MEI across locations and across lags 1, 2 and 3, because these were the closest lags corresponging to the
...
<!--***DAN: insert a specific referece to a wavelet result here-->
  These results again supported our wavelet-based conclusion that deer and MEI are meaningfully related. 

Finally, we did a simple, unlagged correlation analysis of the relationship between deer and DVCs. 
...
<!--***DAN: insert a specific referece to a wavelet result here-->
  The $p$-value result was `r round(cor_pvals_dvcs_deer["p_onetailed_stat1"],4)`, and the $p$-value result of the
...
These results again supported our wavelet results.

\subsubsection{Deer and MEI}\label{Deer_MEI_FMT_results}
<!--***DAN: you'll first need evidence that our Fourier Moran framework is suitable for these data
(which may include the prelim stuff in Review2_Analysis_CoherenceSupport.R) and then you will need to 
present the results for deer and MEI. Actually I now think this first step won't be necessary here, 
as it will have been taken care of in a methods section above. Same for the subsequent sections.-->
...
results, both in the general result that MEI can explain deer synchrony across the $3-7$-year timescale range, and in 
the specific results that MEI is a better explainer of deer synchrony in the $4-7$-year sub-range than it is in the
$3-4$-year sub-range.

\subsubsection{Deer and snow depth}\label{Deer_snow_FMT_results}
<!--***DAN: you'll first need evidence that our Fourier Moran framework is suitable for these data
(which may include the prelim stuff in Review2_Analysis_CoherenceSupport.R) and then you will need to 
present the results for deer and MEI-->
...
results, both in the general result that snow depth can explain deer synchrony across the $3-7$-year timescale range, and in 
the specific results that snow depth appears to be a better explainer of deer synchrony in the $3-4$-year sub-range than it is in the
$4-7$-year sub-range.
...
The plot of $\rho_w^{(\epsilon^{(1)})}$ was in the upper part of the distribution of the surrogate quantities
$\rho_w^{(\delta_j^{(1)})}$ for many frequencies in the $3-y$-year timescale range. 
...
\noindent Total spectral power in the $3-7$-year timescale band for detrended state-total
...
(see SI section{sect:FourierCycles}) for deer revealed that synchrony, $\rho_w$, contributed much more to 
