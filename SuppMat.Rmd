...
<!--***DAN: Tom email on 10/24 indicating code for S10 still needed to be added now that we have split from reumannplatz, so Dan is not sure if Fig S10 is accurate or what at this point.-->
...
generally considered necessary to make statements about fractions of synchrony explained. Although snow depth and deer abundance results on $3-4$-year timescales at county level do not satisfy this requirement, we nevertheless presented the result in the main text that snow depth explains $40\%$ 
<!--***DAN: Tom, can you make this 47 a dynamic link to the correct variable, please?-->
of the synchrony in deer abundance at county level; these variables did meet the requirement at district level, where cross terms were generally smaller (see below for reasons), and deer abundance synchrony explained by snow depth was even higher ($47\%$) at that level.
<!--***DAN: Tom, can you make this 47 a dynamic link to the correct variable, please?-->
Cross terms associated with a spatial wavelet coherence between a spatiotemporal variables and a non-spatial variable such as one of the climate indices were always $0$ by mathematical necessity [@Sheppard2016_aphids].

<!--***DAN: put something about how problems with cross terms will should get less at larger spatial scales, and what that means and why, but need to think this through and also talk to lawrence about it first-->

<!--***DAN: the below needs work-->
We calculated the amount of deer abundance 
...
effect of maximizing the spatial wavelet coherence between 
...
Figs 2\ref{Fig2_DeerSync}C, \ref{FigS6_WMFAndPredictionsDistricts}E, colors). 
...
series (Fig. 4\ref{Fig4_StateWideFluctuations} and associated text), 
...
attributable to synchrony (Fig. 4\ref{Fig4_StateWideFluctuations}).
...
DAN: put plugs in here about how all the data and code and everything are available, and explain how you can regenerate the whole project, and mention wsyn (especially if we can get it on CRAN in time). Also insert plugs at the end of each section above, i.e., mention where the data described in the given section can be found, or where the code implementing a given technique can be found.
...
(Table \ref{TabS4_SummaryDistrict}); these three 
...
quarter-cycle phase shifts (Table \ref{TabS4_SummaryDistrict}). 
...
synchrony at $`r round(usda_abunsnwd_syncexp34,0)`\%$ for $3-4$-year timescales, as 
compared with $`r round(snowabunSyncExp34,0)`\%$ at the county-level. Combined, 
snow depth, MEIw, and PDOw explained $`r round(usda.abunmod_se[[1]],0)`\%$ of the 
synchrony in deer abundance using 
USDA district data (Fig. \ref{FigS6_WMFAndPredictionsDistricts}E).
...
timescales (Fig. \ref{FigS6_WMFAndPredictionsDistricts}B,D). As with 
counties, deer abundance was a significant 
predictor of DVC synchrony (Table \ref{TabS4_SummaryDistrict}), 
and explained a substantial fraction ($`r round(usda_dvcabunSyncExp,0)`\%$) of 
the synchrony (Fig. \ref{FigS6_WMFAndPredictionsDistricts}F). This value 
...
(Table \ref{TabS1_CountyCoherenceSummary}). The renders the estimate of synchrony explained uniterpretable (*Methods*). 
...
abundance remained high ($`r round(usda_adjdvcabunSyncExp,0)`\%$) even after 
...
$`r round(usda_dvcabunSyncExp,0)`\%$ of DVC synchrony was straightforwardly 
interpretable.
...
($P = `r round(ann.dvc.abun_pval,4)`$), with most of the 
significance occurring over 
the $3-5$ year part of the timescale average 
($P = `r round(ann.dvc.abun_pval35,4)`$). DVCs were not 
significant with abundance after correcting for 
traffic ($P = `r round(ann.adjdvc.abun_pval,4)`$). Snow depth, 
MEIs, MEIw and PDOw were all significantly 
coherent with deer abundance over the $3-7$ 
timescale band (Snow: $P = `r round(ann.abun.snow_pval,4)`$; 
MEIw: $P = `r round(ann.abun.wmei_pval,4)`$; MEIs: 
$P = `r round(ann.abun.smei_pval,4)`$; PDOw: 
$P = `r round(ann.abun.wpdo_pval,4)`$). Deer hunters and deer 
abundance were not significantly coherent at the 
state level ($P = `r round(ann.abun.hunter_pval,4)`$).
...
\caption{Wavelet mean fields (left column) and wavelet phasor mean fields (right column) of winter Tmin (A, B), Tmax (C, D), snow depth (E, F) and precipitation (G, H) using county-level data. Colors and contours as in Fig \ref{Fig2_DeerSync}A,B. \label{FigS7_WMFsAndWPMFsAncillary}}
\end{figure}
