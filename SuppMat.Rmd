...
##Tom: the snow depth pvalue would be the variable weatherpvals$Snwd.Abun

2) the name of an R data frame that contains Table S1, e.g., you might just call it TableS1df. 
##Tom: I just called TableS1. Likewise, TableS2 and TableS3 are named after themselves- both of the latter are matrices as that was easier to store things given my current code set up- let me know if that will be an issue. 

3) Please open Endnote and try to figure out how to expport your library of ciations that you are using
for this paper as a bibtex file (.bib extension). This reference may help: https://www.reed.edu/cis/help/LaTeX/EndNote.html. Then commit and push the file to the repo in the same folder as this Rmd and let me know. ##Tom: Done
-->
...
(1981-2016`minyear`-`r maxyear`; Fig. S1<!--***DAN use \ref{<label>}-->). 
...
county (N = 72 `r length(unique(cty.deer$COUNTY_NAM))`). Because of this, and because other 
...
variables, leaving 71`r length(unique(dat$COUNTY_NAM))` counties for statistical analysis.

We obtained estimated hunter abundance data for each DMU from the WIDNR 
that spanned the years 1992-2016`r maxyear`
...
<!--***I dobout the whether there is any data on your last statement, how many go early versus late. I'm not sure I follow the gun v other types of hunters argument- do you mean that the majority of deer get killed by gun hunters and thus that would be the best estimate of hunters for regulating deer?-->
...
years 1987-2016`r maxyear`, from the WI Department 
...
in order to retain the full temporal extent: Tmin ($N=$ 62 `r dim(winter.clim$Tmin[!is.na(rowMeans(winter.clim$Tmin)),])[1]`
counties), Tmax ($N=$ 62`r dim(winter.clim$Tmax[!is.na(rowMeans(winter.clim$Tmax)),])[1]`), precipitation 
($N=$ 64`r dim(winter.clim$Prcp[!is.na(rowMeans(winter.clim$Prcp)),])[1]`), snow depth ($N=$ 60`r dim(winter.clim$Snwd[!is.na(rowMeans(winter.clim$Tmin)),])[1]`). 
We also calculated a Winter Severity Index (WSI, $N=$ 44`r dim(winter.clim$WSI[!is.na(rowMeans(winter.clim$WSI)),])[1]`), 
...
the timescale considered (Fig. 1<!--***DAN-->). 
...
(Fig. S8<!--***DAN-->). Here, wavelet transforms 
...
influence deer abundance (Fig. S1<!--***DAN-->), 
and the 
...
series (Fig. 4<!--***DAN--> and associated text), 
...
attributable to synchrony (Fig. 4<!--***DAN-->).
...
(Fig. 4<!--***DAN-->) of 2629, and if we assume 
...
quarter-cycle phase shifts (Table S4<!--***DAN-->). 
...
synchrony at $34\%$ for $3-5$-year timescales, as 
compared with $15\%$ at the county-level. Combined, 
snow depth, MEIw, and PDOw explained $90\%$ of the 
synchrony in deer abundance using 
USDA district data (Fig. S6E<!--***DAN-->).
...
timescales (Fig. S6B,D<!--***DAN-->). As with 
counties, deer abundance was a significant 
predictor of DVC synchrony (Table S4<!--***DAN-->), 
and explained a substantial fraction ($80\%$) of 
the synchrony (Fig. S6F<!--***DAN-->). This value 
...
(Table S1<!--***DAN-->). The renders the estimate of synchrony explained uniterpretable. 
<!--***DAN: need a citation to some other point in the ms, e.g., Methods-->
...
abundance remained high ($73\%$) even after 
...
$80\%$ of DVC synchrony was straightforwardly 
interpretable.
...
<!--***DAN: the abundance/summer MEI synchrony explained and cross term values appear to be missing from the R variable TableS1 and need to be put in in the R code-->
...
knitr::kable(TableS1,
             ...
             caption = "Summary of spatial coherence tests and related results using county-level data (\\emph{Methods}). Each row is based on a test of spatial coherence of the variables in the first two columns. Tests were across the $3-7$-year timescale band, except for ancillary results in the footnotes. $P$-values are provided for all tests, and average phases over the timescale band used, fractions of synchrony explained, and average cross terms (\\emph{Methods}) are provided where meaningful (for significant tests, $P < 0.05$, or marginally significant tests). A positive mean phase indicates the response is leading, and a negative mean phase indicates the predictor is leading. Cross-terms are zero for the climate indices because they used the same values for all locations. All values were rounded so that four digits were displayed.\\label{TabS1_CountyCoherenceSummary}",
             booktabs=T)
```

\noindent $^1$For $3-5$-year timescales, $p = 0.028$<!--***DAN: -->, phase $= 0.871$<!--***DAN: -->, and synchrony explained $= 0.288$.<!--***DAN: -->

\noindent $^2$For $2-2.5$-year timescales, $p = 0.005$<!--***DAN: -->, phase $= -0.827$<!--***DAN: --> and synchrony explained $= 0.001$<!--***DAN: -->.

\newpage

<!--***DAN: once a variable exists, can uncomment this. It seems TableS2 does not exist even though Tom's email says it does.-->
```{r TableS2,echo=F,results='markup'}
#knitr::kable(TableS2,
#             col.names=c("Winter NAO","Winter PDO","Winter MEI","Summer NAO","Summer PDO","Summer MEI"),
#             format='pandoc',
#             caption = "$P$-values of spatial coherence between climate indices. Values inside parentheses indicate the mean phase difference #($\bar{\theta}$) between variables over $3-7$-year timescales (only shown for pairs of variables that were both significantly coherent [$p < #0.05$] with deer abundance). \\label{TabS2_PairwiseClimVarSpatCoh}",
#             booktabs=T)
```

\newpage

<!--***DAN: once a variable exists, can uncomment this-->
<!--***DAN: Tom, is this for 3-7-year timescales? The caption should say, please add-->
<!--***DAN: Use Prcp or Precip (does not matter) consistently throughout paper-->
```{r TableS3,echo=F,results='markup'}
rownames(TableS3)<-c("Tmin","Tmax","Precip","Snow depth","WSI")
knitr::kable(TableS3,
             format='pandoc',
             col.names=c("Winter NAO","Winter PDO","Winter MEI","Summer NAO","Summer PDO","Summer MEI"),
             caption = "$P$-values of spatial coherence between climate indices and average winter weather conditions using county-level data. \\label{TabS3_SpatCohPWWAndClimInds}",
             booktabs=T)
```
...
#knitr::kable(,#Insert name of R dataframe containing table S4 here
#             format='pandoc',
#             caption = "Summary of spatial coherence tests and related results using USDA district-level data (\emph{Methods}). Each row is based on a test of spatial coherence of the variables in the first two columns. Tests were across the $3-7$-year timescale band, except for ancillary results in the footnotes. $P$-values are provided for all tests, and average phases over the timescale band used, fractions of synchrony explained, and average cross terms (\emph{Methods}) are provided where meaningful (i.e., for significant tests, $P < 0.05$). A positive mean phase indicates the response is leading, and a negative mean phase indicates the predictor is leading. Cross-terms are zero for the climate indices because they used the same values for all locations. \\label{TabS4_SummaryDistrict}",
#             booktabs=T)
```
...
$^1$For $3-5$-year timescales, $p = 0.028$<!--***DAN: -->, phase $= 0.860$<!--***DAN: -->, and synchrony explained $= 0.339$.<!--***DAN: -->

\noindent $^2$For $2-2.5$-year timescales, $p = 0.012$<!--***DAN: -->, phase $= -0.326$<!--***DAN: --> and synchrony explained $= 0.002$.<!--***DAN: -->
...
#knitr::kable(,#Insert name of R dataframe containing table S5 here
#             format='pandoc',
#             caption = "$P$-values of spatial coherence between climate indices and average winter weather conditions using USDA district-level data. \\label{TabS5_SpatCohPWWAndClimInds_District}",
#             booktabs=T)
```
