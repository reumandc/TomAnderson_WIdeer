...
Some readers may perceive a mismatch, here. Though we do not think there is as mismatch, this section and SI section 
...
the theory and wavelet methods for the data analyses of the main text. First, both sets of methods are
spectral methods, and are closely related and serve similar goals. Second, differences
...
assume stationarity of time series. But extensive analytic machinery is also available under these assumptions. 
It was appropriate to develop our theoretical approach under a set of simplifying assumptions that 
made possible the use of powerful mathematical tools.
We then used wavelet methods, which do not assume stationarity and are more suited to the 
complexities of real data, for empirical work.
Although we again emphasize that Fourier and wavelet approaches are so closely related that we do not
see a mismatch, we nevertheless still provide a complete set of Fourier-only analyses for 
readers that remain skeptical.  
...
a fraction $`r round(1-pval_deer_peakiness,4)`$ of the random-permutation surrogates we used. Thus the null 
