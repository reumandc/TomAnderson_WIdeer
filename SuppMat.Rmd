...
<!--***DAN: Make sure you actually use that many when you do this, or change this text-->
...
<!--***DAN: this should have the model setup, the main theorem, the content of the two sections on interptreting
the Fourier Moran theorem for the two specific models, reframed as corrollaries. Also the "statistically 
attributable" section-->

We begin by framing two specific population models, followed by a general mathematical framework which covers both models
as special cases.
...
by $\epsilon^{(2)}$.
<!--***DAN: I have not thought about doing this Moran work for deer causing DVC synchrony yet, do I 
...
\subsubsection{How to tell whether the framework is suitable for our data}
<!--***DAN: One thing that probably goes in here is the correlation and Foueri results you produced early after getting the
second round of reviews which show relationships between snow and MEI and deer. Or maybe that goes in the corresponding results
section, but the methods go here. Another thing is the modelling which actually
fits model of deer with drivers MEI and snow. Though you have a bit more work to do to complete that - you need to validate that the 
AIC-best model you selected is actually an OK model (hopefully it is).-->

\subsubsection{Numeric and significance procedures associated with the first specific model}
<!--***DAN: You have written it, but you need to add details to make it transition at the end to being
more like a methods section specifying exactly what we're doing with our actual data--> 
<!--***DAN: this and the next section have to be written under the assumption that the framework is
validated to be suitable for the data, since we will have presented the methods of the previous 
section at this stage, but not the results-->

\subsubsection{Numeric and significance procedures associated with the second specific model}
<!--***DAN: You have written it, but you need to add details to make it transition at the end to being
more like a methods section specifying exactly what we're doing with our actual data--> 
...
\subsubsection{Deer and snow depth}
<!--***DAN: you'll first need evidence that our Fourier Moran framework is suitable for these data
(which may include the prelim stuff in Review2_Analysis_CoherenceSupport.R) and then you will need to 
present the results for deer and MEI-->


\subsubsection{DVCs and deer}
<!--***DAN: you'll first need evidence that our Fourier Moran framework is suitable for these data
(which may include the prelim stuff in Review2_Analysis_CoherenceSupport.R) and then you will need to 
present the results for deer and MEI-->
