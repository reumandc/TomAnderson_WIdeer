...
for hunter numbers across the season. Gun hunters on average represent `r round(100*mean(state.totals$GunHunters[state.totals$Year>1980]/(state.totals$GunHunters[state.totals$Year>1980]+state.totals$BowHunters[state.totals$Year>1980])),0)\%` of the total hunters in WI each year over the years of this study.
...
<!--***DAN: It was Robert that told us this, and he is a coathor, so perhaps we should take out the personal communication parens? It would be best if we could get a citation for this, even a white paper, if there is one and we can fit it in. If we leave it as "personal communication" then that begs the question "personal communicccation with whom?" The WIDNR is not a person.-->
...
Also, and importantly, when you rerun with 100000 (if indeed we do - not clear to me it's strictly necessary) we will have to re-check that all statements are consistent with the new results, because all the results will change. They should not change much, but they will change. Will have to review for this.-->
...
<!--***DAN: Tom, see previous comment. I also changed the line right above this to be an automated link-->
...
traffic ($P = `r round(ann.adjdvc.abun_pval,4)`$). -->
$\text{MEI}_{\text{s}}$, $\text{MEI}_{\text{w}}$ and $\text{PDO}_{\text{w}}$, but not snow depth, were significantly 
coherent with deer abundance over the $3-7$-year 
timescale band (snow depth, $P = `r round(ann.abun.snow_pval,4)`$; 
$\text{MEI}_{\text{w}}$, $P = `r round(ann.abun.wmei_pval,4)`$; $\text{MEI}_{\text{s}}$, 
$P = `r round(ann.abun.smei_pval,4)`$; $\text{PDO}_{\text{w}}$, 
$P = `r round(ann.abun.wpdo_pval,4)`$). 
<!--***DAN: Tom: please add a test for snow depth on 3-4-year timescales-->
Hunter abundance and deer 
abundance were not significantly coherent at the 
state level on $3-7$-year timescales ($P = `r round(ann.abun.hunter_pval,4)`$).
<!--***DAN: Tom, pls add a test for 2-2.5-year timescales-->
...
<!--***DAN: Tom, can you please remove the blue cross-hatching on panel G, as there is no corresponding mean phase given for that range (and there should not be a mean phase given since the coherence was not significant over that range).-->
