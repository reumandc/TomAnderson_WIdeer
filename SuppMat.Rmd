...
compared with $`r round(100*abun_snwd_syncexp3_4,0)`\%$ at the county-level. Combined, 
...
knitr::kable(TableS1,
             format='pandoc',
             digits=4,
             col.names=c("Response","Predictor","Timescale", "P-value","Mean phase ($\\bar{\\theta}$)","Synchrony explained","Average cross terms"),
...
"P-values of spatial coherence between..." but these are all wavelet coherences (or should be) not spatial coherences, since these are all single time series. I changed it, but Tom can you please go back to the code and look and see whether you actually computed spatial coherences with spatially replicated time series or wavelet coherences between single time series? Should be the latter. It may (or may not) turn out to make a difference, I am not sure, but we should anyway do the appropriate thing. See also the point in the main text in the para that begins "Spatial wavelet coherence, wavelet regression techniques...", in which a value from this table is quoted (the one for Winter MEI and Winter PDO). I added a note there that it was a wavelet coherence result (not a spatial coherence result), so it needs to be checked that it is.-->
