...
\textbf{Use of Fourier methods for the theory and wavelets for the empirical analysis}. Fourier and
wavelet methods were used principally
for our theoretical and empirical analyses, respectively, though some Fouier analyses were also used
for our empirical analyses. It was appropriate, for several reasons, to use Fourier methods for 
the theory and wavelet methods for the data. First, both sets of methods are in the broader category of 
spectral methods, and
Fourier and wavelet approaches are related. So there is no actual mismatch produced when wavelet methods
are used to empirically test theory that was developed with Fourier methods.
For instance, the wavelet spectrum gives the same information as the Fourier spectrum, but without 
the necessity of assuming stationarity. And the wavelet transform is a filtered cross spectrum.
Wavelet methods and Fourier methods often serve very similar goals. Second, differences
that do exist between Fourier and wavelet approaches are consistent with differences between our 
theoretical and empirical analyses. Fourier analyses make more structural assumptions 
about the data, notably, they
assume stationarity. At the same time, various techniques of mathematical analysis are 
commonly used for Fourier approaches, under an assumption of stationarity. We developed 
our theory in an idealized setting, for which Fourier methods and 
mathematical analysis were possible, but then used
wavelet methods, which are more suited to the complexities of real data, for our empirical work.

# Supplementary results \label{SI:results}

\textbf{Fraction of deer synchrony explained by winter MEI, county data, technical results.} Of the parameters 
...
deer synchrony explained by winter MEI may have been inflated. Averaging across the parameter sets used, 
...
estimate, based solely on the wavelet Moran theorem, that winter MEI explained $`r round(abunwmeiSyncExp37)`\%$
...
fraction of deer syncrony explained by winter MEI was inflated. See SI section \ref{SI:methods}. 
