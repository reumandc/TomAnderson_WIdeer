...
See Reinsell [-@Reinsel1997], Shumway [-@Shumway2000], and Brillinger and Stoffer [-@Brillinger2001]
...
section 1.1.3 of Reinsel [-@Reinsel1997]. But 
$T(\mathcal{C}) = (1-\lambda)I$ and $T(\mathcal{Q})=\gamma I$
(section 1.1.3 of Reinsel [-@Reinsel1997]), so
\begin{align}
...
We calculated deer abundance estimates based on data from deer management units provided by the Wisconsin Department of Natural Resources (WIDNR) spanning the `r dim(cty.list$Abun)[2]`-yr period `r minyear`-`r maxyear` (Fig. \ref{MT-Fig2_Timeseries}) for `r length(unique(dat$COUNTY_NAM))` counties. From 1981-2013, deer abundance was estimated for deer management units 
...
(Fig. \ref{MT-Fig2_Timeseries}); these data constituted an index of the estimated 
...
of Transportation (WIDOT; Fig. \ref{MT-Fig2_Timeseries}). These data reflect the number of police-investigated DVCs; they are an 
...
all subsequent analyses (Fig. \ref{MT-Fig2_Timeseries}). The number of weather stations per county and data collected at each station 
...
(Fig. \ref{MT-Fig2_Timeseries}). We used a total of 11 weather and 
...
We performed an optimal Box-Cox transformation on each time series to help ensure Gaussian marginal distributions [@Sheppard2016_aphids], which are necessary for the Fourier-surrogate-based spatial wavelet coherence and wavelet linear regression testing procedure described below. This process also included linear detrending (and subtracting the mean) to remove any long-term trends that might otherwise obscure patterns of synchrony [@Buonaccorsi2001]. Time series
...
<!--***DAN: Tom, please check and fix how this references comes out, which is wonky - the callout is Team (2018)! I also think we should be citing the package, here, not R - see my comment in the
main text-->
...
($P < 0.001$, one-tailed test, see contours on, e.g., Figs \ref{MT-Fig3_DeerSync}B and \ref{MT-Fig4_DvcSync}B), synchrony was 
...
Fig. \ref{FigS8_WMFsAndWPMFsAncillary}. Wavelet mean fields were computed using the
...
(Fig. \ref{FigS4_PhaseDiffs}). Spatial wavelet coherences were computed and results were processed using the `coh`, `bandtest`, `plotrank`, `plotphase` and related functions in the `wsyn` package. The fast algorithm of  @Sheppard2017_surrogates for assessing the statistical significance of spatial wavelet coherence was used.

Correlation-based approaches have been standard practice in synchrony studies [@Buonaccorsi2001], but these methods assume in-phase relationships. Correlation-based approaches also conflate timescales, and recent results show this can hinder detection of synchronous phenomena by obscuring important synchrony on some timescales with asynchronous or noisy dynamics on other timescales [@Sheppard2016_aphids]. Moreover, synchrony must be specific to certain timescales to produce cyclic dynamics of total populations on those timescales (Fig. \ref{MT-Fig1_example}). 
...
influence deer abundance (Fig. \ref{MT-Fig2_Timeseries}), 
and the 
...
series (Fig. \ref{MT-Fig5_StateWideFluctuations} and associated text), 
...
that was attributable to synchrony (Fig. \ref{MT-Fig5_StateWideFluctuations}).
...
(Fig. \ref{MT-Fig5_StateWideFluctuations}) of `r avgdvcfluc`, and if we assume 
...
abundance at $3-4$-year timescales (Table \ref{MT-Tab1_CountyCoherenceSummary}; see also
...
to deer synchrony over $4-7$-year timescales (Table \ref{MT-Tab1_CountyCoherenceSummary}, see also 
Fig. \ref{FigS5_RanksInSurrogs}A and SI Table \ref{TabS2_SummaryDistrict}). 
$\text{MEI}_{\text{w}}$ and $\text{PDO}_{\text{w}}$ explained $`r round(abunwmeiSyncExp47,)`\%$ and $`r round(abunwpdoSyncExp47,0)`\%$ of deer synchrony on $4-7$-year timescales, respectively (Table \ref{MT-Tab1_CountyCoherenceSummary}). Snow depth did not explain $4-7$-year synchrony: spatial wavelet coherence between snow depth and deer abundance on those timescales was not significant (Table \ref{MT-Tab1_CountyCoherenceSummary}, \ref{TabS2_SummaryDistrict}). The timescale
