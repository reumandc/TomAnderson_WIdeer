...
background information can be found from many sources, including the books of
...
So the cospectrum, denoted $C_{\epsilon_{i}\epsilon_{j}}$, is $\Rp(S_{\epsilon_{i}\epsilon_{j}})$, 
the real part. Analogously, let $S_{w_i w_i}$,
...
Give a multivariate process such as $\epsilon=(\epsilon_1,\ldots,\epsilon_N)$ or
$w=(w_1,\ldots,w_N)$, the spectral matrix of the process is denoted
$S_{\epsilon \epsilon}$ or $S_{ww}$. The spectral matrix, an $N \times N$ matrix, 
has the spectra
$S_{\epsilon_i \epsilon_i}$ on the diagonal and the cross spectra $S_{\epsilon_i \epsilon_j}$ 
in the off-diagonal. We will later use the
...
respectively, $S_{\epsilon_{ij},\epsilon_{ij}}$ and $S_{w_{ij}w_{ij}}$.
