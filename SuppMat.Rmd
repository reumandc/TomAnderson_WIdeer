...
```{r echo=FALSE, results="hide", cache=TRUE, fig.show="hide", cache.extra=list(mtime("Data/abunsurrsum.csv"),mtime("Data/dvcsurrsum.csv"),                                                                                mtime("Code/FourierAnalysesDeer.R"),mtime("Code/FourierAnalysesDVCs.R"),mtime("Results/cty.list.rds"),mtime("Results/cty.list.rds"))}
source("Code/FourierAnalysesDeer.R")
source("Code/FourierAnalysesDVCs.R")
```


<!--***DAN: The below chunks should eventually be consolidated into the above chunks-->


<!--The new Fourier and correlation analyses developed in response to the same reviewer, second review, 2020 07-->
```{r echo=FALSE, results="hide", cache=TRUE, fig.show="hide", cache.extra=list(mtime("Code/Review2_Analysis_CoherenceSupport.R"),mtime("Code/Review2_Analysis_FourierDecomp.R"),mtime("Results/cty.list.rds"),mtime("Results/winter.clim.rds"),mtime("Results/climindex.rds"))}
source("Code/Review2_Analysis_CoherenceSupport.R")
source("Code/Review2_Analysis_FourierDecomp.R")
```
...
# Alternative analysis: methods \label{sect:alt_methods}
...
These quantities were estimated from available data using the consistent estimator of @Brillinger2001 (section 5.6 
of that text). 
...
contribitions (equation (\ref{MT-mainresmt}) of the main text) should reveal much greater cospectral contributions to the 
peak of $S_{\text{tot}}$ on $3-7$-year timescales. We tested this 

The equation 
\begin{equation}
S_{\text{tot}}=\sum_{i,j} C_{w_i w_j} = \sum_i C_{w_i w_i} + \sum_{i \neq j} C_{w_i w_j}
\end{equation}
follows straightforwardly from the definitions of the spectra and cospectra of the population processes $w_i$ in 
the locations $i$ and the spectrum, $S_{\text{tot}}$, of $w_{\text{tot}}=\sum_i w_i$ (see )
<!--***DAN: Obviously this is someting you started and did not finish yet-->
...
<!--***DAN: The rest of this section is material from the last version, stored here for now-->

<!--Fourier results-->
\subsection{Fourier results} \label{SI:results:Fourierresults}
