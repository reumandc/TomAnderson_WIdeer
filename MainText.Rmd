...
unrelated to each other ([@Theiler1992;@Schreiber2000;
@Sheppard2013;@Sheppard2016_aphids]). We calculated how often observed spatial
wavelet coherence between pairs of variables was ranked higher than coherences of
surrogate data sets, aggregating across the timescale band of interest using an
...
synchrony, following the wavelet Moran theorem of *et al.* [@Sheppard2016_aphids].
We also examined the amount of deer synchrony jointly explained by snow depth,
MEIw, and PDOw (the three primary drivers of deer synchrony; see Results) using
...
$3-7$ timescale band of interest. We accomplished this by randomizing the phases
of the Fourier transforms in the surrogate data sets. We summed each set of
...
<!--Tom: I think we can delete the follwoing sentence; most R packages have vigenettes so this doesn't seem to add much to me. The `wsyn` package includes a vignette documenting how to use the functions implemented there, and giving examples; the vignette provides additional description of the methods of this paper.--> All data and code producing this paper are archived at `www.github.com/reumandc/TomAnderson_WIdeer`, including the complete revision history of the process that led to the presented analyses.
...
abundances tended to peak in years of below-average snow. No other weather variables were statistically significant (\ref{SM-TabS1_PairwiseClimVarSpatCoh}) with deer abundance. Wavelet mean field and 
...
See Fig. \ref{Fig3_DeerSync}C for a visual manifestation of this statistical result. None of the other climate indices were related to deer abundance (\ref{SM-TabS1_PairwiseClimVarSpatCoh}). 
