...
section \ref{SM-SI:methods}; and complete details of all methods are in @Sheppard2013 and
@Sheppard2016_aphids. It is explained in SI section \ref{SM-SI:methods}
...
phasor mean field magnitudes (SE section \ref{SM-SI:methods}; SI Fig.
...
(SI section \ref{SM-SI:methods}; SI 
Fig. \ref{SM-PedagogFigSpatCoh}; @Sheppard2013, @Sheppard2016_aphids, @Sheppard2017_surrogates). 
...
average-rank-based method described in detail in SI section \ref{SM-SI:methods} and 
by @Sheppard2016_aphids. A p-value 
...
for our research goals, so were relegated to SI section \ref{SM-SI:methods} for
methods and \ref{SM-SI:results} for results.
<!--***DAN: don't forget to add this material, and mention there that those results are 
...
seeing how they answer the questions, in the Results.-->
...
applied to obtain estimates of percentages of synchrony in deer explained by $\text{MEI}_{\text{w}}$,
because $\text{MEI}_{\text{w}}$ is not a spatially resolved variable (*Methods*).
Such estimates were complicated, and artefactually inflated. Furthermore, such estimates were not necessary
to substantiate our argument that deer are environmentally synchronized in the $3-7$-year
band, since this is adequately demonstrated by our wavelet coherence results between deer and MEI,
and our results for snow depth. Thus such estimates were placed in SI section
...
clearly (SI section \ref{SM-SI:results}) that: 1) there was a meaningful spectral peak in the
...
2) the same spectral peaks did not occur in county-level deer time series, i.e, combining 
...
produce population cycles. Previous attemps to link environmental factors to population cycles have either been unsubstantiated [e.g., sun spots;
@Sinclair1992_sunspots;@Selas2004_sunspots], or provided a useful but patial explanation for cycles, as in studies of 
tetraonids [@watson2000] or small mammals [@kausrud2008] which demonstrated that environmental factors tend to modify cycles 
produced by density dependent factors, rather than producing them outright. Our study differs from some of these 
previous attempts in part because the new mechanism does not require a highly regular environmental 
oscillator such as sun spots. Rather, it is sufficient simply for irregular environmental drivers 
...
an additional example of Moran effects causing synchrony [@Leibhold2004_sync;@hansen2020]. 
Finally, we have presented what is, to our knowledge, the first demonstration that synchrony 
...
to human systems. Thus if climate change alters synchrony, as a growing number
...
our results show that large-spatial-scale cycles
...
<!--***DAN: It used to say "an important but previously unrecognized applied motivation", so 
this change contributes to efforts to tone down the claims of how important this is in the applied world.
I put this note so I don't forget I made this change when I write responses to referees.-->
...
3-year population synchrony and cycling; scenario 3 has environmental synchrony on a 10-year timescale, resulting in
10-year population synchrony and cycling. In scenario 1, the asynchronous environment scenario,
...
standard normal and independent across locations and times. In scenario 2, the 3-year synchrony scenario,
...
Noise for scenario 3, the 10-year synchrony scenario (C), 
...
\label{Fig1_example}}
\end{figure}
<!--***DAN: Tom, in the above caption, at the referee's request, I introduced names for the three 
scenarios. To correspond with these names, can you please make the header titles at the top of each column 
in the figure itself read as follows: "Scenario 1: asynchronous environment", "Scenario 2: 3-year synchrony", "Scenario 3: 10-year synchrony". You may need to reduce the font size of these titles or use two lines to make them fit.
Also have a look at my revisions to the caption, which are intended for greater clarity at the referee's request.-->
