...
Supplementary Fourier approaches are described in 
SI section \ref{SM-SI:methods}, and use standard techniques.
Fourier approaches
were used, for instance, to examine: 1) whether the spectral power in the state-total deer time series
in the $3-7$-year timescale band is greater than it would be without synchrony in that band; and
whether 2) the same spectral peaks in the $3-7$-year band do not occur in county-level deer time series.
...
Such estimates were complicated, artefactually inflated, and not necessary
for our argument that deer are environmentally synchronized in the $3-7$-year
band, since this is adequately demonstrated by our wavelet coherence results for deer and $\text{MEI}_{\text{w}}$,
and our snow depth results. Thus such estimates are in SI section
\ref{SM-SI:results}, where a quantification of the degree of inflation is also described 
