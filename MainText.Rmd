...
<!--***DAN: don't forget to fill this stuff in-->
...
# Summary of theory
<!--***DAN: Tom, after reading and thinking, I think this is better as a separate section.
...
as methods, and methods tend to get minimized, whereas in my opinion theory should
not be minimized.-->
...
autoregressive moving average (ARMA) dynamics within a habitat patch, and $F_{\text{popdyn}}(f)$
...
and year. This measure is known to relate to winter impacts on deer populations [@widnr2001].
...
package for the R programming language [@Rstats2018], which is itself well documented.
...
wsyn citation. I know, the R people want you to cite R, but I, personally, think we have gone
beyond that and they are just being greedy for citations in the same way that we would 
view Decarte as being greedy if he was alive and wanted to try to insist we cite him
whenever using a cartesian coordinate plane.-->
...
<!--***DAN: Tom, we cannot cite the results as an example of the methods!
...
We calculated the percentage of deer synchrony explained by individual weather and
climate variables, as well the percentage of DVC synchrony explained by deer
...
like if the only drivers of synchrony were the predictors. This predicted synchrony
...
change in the location of the above text. Might as well do it all at once.-->
...
Snow depth and climate index results indicated (SI section \ref{SM-SI:results}) 
distinct or partly distinct phenomena 
synchronizing deer on different timescales within the $3-7$-year range. NAO was not 
...
(Table \ref{Tab1_CountyCoherenceSummary}, Figs \ref{SM-FigS5_RanksInSurrogs}B, \ref{SM-FigS6_WMFHunter}). Hunter abundance was significantly related to deer abundance on shorter timescales, around 
...
\ref{SM-FigS5_RanksInSurrogs}B). But these short-timescale results, while interesting
(*Discussion*), are not 
particularly relevant for synchrony because deer showed little synchrony on 
...
assumptions made about costs and reporting rates of DVCs (SE section \ref{SM-SI:methods}), 
statewide DVC damages 
vary by US\$$`r floor(round(2*avgdvccostsLow,-6))/1000000`-`r floor(round(2*avgdvccostsHigh,-6))/1000000`$Mn 
between peak and trough years [@Huijser2008_dvc;@Marcoux2010_dvc;@Conover2011_bookchp]. 
...
<!--Variants of the mechanism. This could be moved to sup mat if we wanted to save space
for something else. It is pretty technical. In any case, it needed to be streamlined, so
...
counties to produce statewide fluctuations. Second, the local population oscillations 
...
and musk oxen [@post2002], and thus the cycling mechanism we observed may occur as wel
...
<!--***DAN: Dan thought the connection to geog of sync was too distant to be included in what is otherwise
and very focussed Discussion.
In WI, this situation likely played out to a greater extent in northern areas, where snow depth is greater and access to high quality food resources (i.e., agricultral crops) is available. Determining whether such landscape or habitat differences across regions may further decompose how climatic Moran efects influence the spatial patterning synchrony in deer [@Walter2017_geogsync].-->
<!--***DAN: moved this stuff to first para of Discussion:
Recent work on several taxa 
[@Allstad2015_gypsysync;@Sheppard2016_aphids;@Koenig2016_birdsync;@Shestakova2016_forestsync] shows 
that synchrony of some environmental drivers may be changing, possibly due to climate change. These 
changes may alter population synchrony, 
...
a larger number of synchronized local population accentuates the effect we described.
...
large spatial extents [@brown2000;@simard2013]. The lack of longer-timescale spatial wavelet 
...
Greater deer abundance almost certainly caused
...
The influence of climate change on synchrony is probably important
[@Post2004_greenland;@Allstad2015_gypsysync;@Defriez2016_plankton;
...
influence human activities. Timescale specificity in synchrony is now know to be common 
[@Sheppard2016_aphids;@Sheppard2017_surrogates;@Walter2017_geogsync;@Sheppard_wavmodel;@Haynes_etal2019;
@Anderson2019_LO], making it seem likely our mechanism will be more general than the 
system we studied here.
...
certain timescales. Population cycles would not have manifested in deer had deer synchrony 
not been specific to certain timescales ($3-7$-years).
Additionally, many past analyses of synchrony have 
...
and again here, we showed that using timescale-specific methods can greatly 
ameliorate this difficulty and reveal new phenomena and unrecognized mechanisms.

<!--This is not really a conclusion it is more suggestions for future work. Delete, or relabel?
Or just delete the label - we don't have any other labels in the Discussion. I am currently thinking to 
delete. Commenting for now. It does not say much that is new and specific, just some general
stuff about how it would be good to look for this in other systems.-->
...
grey lines (B for deer, D for DVCs, in thousands). $\Delta$ represents the difference in values from 
