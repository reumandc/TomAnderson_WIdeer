...
for other systems, posited that strongly cyclic environmental phenomena such as sun spots may drive 
population cycles [@Sinclair1992_sunspots;@Selas2004_sunspots]. The sun spot hypothesis was 
refuted, however [@Nilssen2007_nosunspots;@Myers2013_nosunspots], and whereas it is now recognized that environmental drivers 
such as winter weather can alter cycles through their interactions with intrisic population
processes [@watson2000;@kausrud2008], for decades 
research has focused instead on explanations which principally involve nonlinear or delayed density dependence, 
...
Supporting Information (SI) sections \ref{SM-SI:theory}-\ref{SM-sect:mainproof}. The bulk of the paper is 
spent examining empirical evidence for the mechanism.

<!--***DAN: deleted an instance of the word "major" to be more understated and to address a referee concern about overselling the applied significance-->
One applied concern is that if
...
[@addison2002], and Fourier methods, which may be more familiar to some readers, could be used, *post hoc*,
to confirm some results. The methods we use are 
well established, standard tools in many biological fields, or are simple extensions thereof. 
...
<!--***DAN: Add technical details for the below techniques to the sup mat, and then make the references refer to the 
new as well as the existing stuff-->
...
average-rank-based method described in detail elsewhere [@Sheppard2016_aphids]. 
<!--***DAN: to satisfy the referee, also explain this methd in the sup mat, and insert a reference to the sup mat here-->
A p-value 
...
for our main research goals, so were relegated to the SI (section \ref{SM-SI:methods} for
methods and \ref{SM-SI:results} for results).
<!--***DAN: don't forget to add this material-->

<!--***DAN: Make sure there are sufficient technical details for the below techniques in the 
sup mat, and then make the references refer to the new as well as the existing stuff-->
...
mechanism can also be detected in our data using Fourier analyses which may be more familiar to
some readers. Supplementary Fourier approaches are described in 
SI section \ref{SM-SI:methods}. All Fourier approaches we used are standard techniques, used in many fields.
Fourier approaches
were used, for instance, to examine whether: 1) the spectral power in the state-total deer time series
in the $3-7$-year timescale band is greater than it would be without synchrony in that band; and
2) the same spectral peaks in the $3-7$-year band do not occur in county-level deer time series.
Further details of all datasets,
...
<!--***DAN: made it to here in re-reading on 2020 04 15-->
...
Supplementary results using Fourier approaches (*Methods*, SI section \ref{SM-SI:methods}) demonstrated 
clearly that: 1) there was a meaningful spectral peak in the
$3-7$-year timescale band for the state-total deer time series for WI, and it was substantially caused by 
synchrony because when $3-7$-year synchrony was removed from county-level time series the peak was also removed;
2) the same spectral peaks did not occur in county-level deer time series (SI section \ref{SM-SI:results}), i.e, combining 
time series across space produced the peak by combining weak but synchronous fluctuations in the $3-7$-year timescale band.
