...
population cycles [@Sinclair1992_sunspots;@Selas2004_sunspots]. However, support for that hypothesis has been equivocal, as some drivers like sunspots have been refuted [@Nilssen2007_nosunspots;@Myers2013_nosunspots] while others like weather patterns likely contributed at least partially to cycles [@watson2000;@kausrud2008]. Therefore, for decades 
...
<!--***DAN: you need to add some text to SI methods arguing the above statement, you can rephrase from the aphid paper--><!--TOM: Does it make sense to do this here? It seems more like discussion material- it seems rare to me to have citations in a results section generally speaking-->
...
citations, please), in which case get rid of it.--><!--TOM:I've changed all the citations so that they only reference Table 1, and agree we can get rid of Table S1, though i can't figure out how to just commen it out (as opposed to just deleting it; I thought saving the text might be useful but maybe not-->
...
the panels. The caption will have to be changed, but you can leave that to me if you want.--><!--TOM: Removed the figure, and updated the caption-->
-->
...
2-2.5-year hunting coherence results into Table 1--><!--TOM: Done-->
...
produce population cycles. Previous attemps to link environmental factors to population cycles have either fallen short [e.g., sun spots;
@Sinclair1992_sunspots;@Selas2004_sunspots], or provided only a patial explanation for cycles, as in studies of tetraonids [@watson2000] or small mammals[@kausrud2008]. Our study differs from these previous attempts because the new mechanism does not
...
synchrony in that band and leading to total-population cycles on the same timescales. Our results also provide further evidence that Moran effects cause synchrony [@Leibhold2004_sync;@hansen2020], in part because of recently developed methods, such as the timescale-specific methods used here[@Sheppard2016_aphids;@sheppard_wavmodel], that better help identify putative causal drivers. 
...
<!--***DAN: add the citation to the new Grotan review here--><!--TOM: I assume you meant the Hansen paper that has Grotan as an author; I added it if so-->
...
Such fluctuations may influence insurance and accident recovery industries, though we have not examined whether a fluctuation of this magnitude is sufficient, relative to other fluctuations that occur in accident rates, to be of substantial concern..
...
\label{Fig3_DeerSync}}
\end{figure}
<!--TOM: Removed the following from the caption, as its no longer needed: Colors on (C) show synchrony in deer abundance predicted from a wavelet linear model (\textit{Methods}) using 
