...
described elsewhere [@Sheppard2013;@Sheppard2016_aphids] and below we provide operational descriptions. All analyses were done via functions in the `wsyn` package for the R programming language [@Rstats2018].
...
and timescale. When the magnitudes of these complex functions are plotted against time and timescale, the plots characterize the degree 
...
technique ([@Sheppard2013;@Sheppard2016_aphids;@Sheppard2017_surrogates]; SI Appendix Fig. \ref{SM-PedagogFigSpatCoh}). Spatial wavelet coherence tests whether two 
...
(Fig. \ref{SM-FigS4_PhaseDiffs}). 

Spatial wavelet coherence allows for any kind of phase relationship between variables, as opposed to only in-phase relationships that correlation methods use, and provides as part of its output the average phase that occurred between two variables. Moreover, spatial wavelet coherence is also  timescale-specific. Becayse synchrony must be specific to certain timescales to produce cyclic dynamics of total populations on those timescales (Fig. \ref{Fig1_example}), this method is better suited for assessing phase relationships.  Therefore, when two variables had significant spatial wavelet coherence over a timescale band, we 
...
coherence between deer abundances and DVCs (see *Results*) is likewise probably 
...
abundances tended to peak in years of below-average snow. Deer abundance was also related, with
